...
work: ROADMAP §10.2, §10.11, §12.5, and §13.9. v2: the requirements in §15,
and ROADMAP §14.8.
...
- Recovery is "read both supers, take the highest generation whose checksum
  matches." v2 also checks the tree roots the newer one names and takes the
  older when they fail (§15). No journal replay in the kernel, no
  torn-journal special cases.
...
`write` cannot mix old and new under the committed tree. v2's NOCOW files
(§15) are the one exception, for their own data only.
...
4. For each inode: kind/mode sanity, extent ranges in-volume and allocated,
   ...
   dirent count (root nlink is 1 in v1; subdirs do not store `..`). From v2,
   a directory's `nlink` is 2 plus its subdirectories, a device inode has an
   `rdev`, and an inode whose `nlink` is 0 is on the orphan tree and in no
   directory (§15).
...
mount it after a rollback. A release therefore never sets an incompat or
ro_compat bit on an existing volume on its own. The one exception is the
intent log's `log_replay` bit, which a volume carries only while its log may
hold records (§15's Commit row). Enabling any other such feature is an
explicit admin command that names the oldest release still able to mount
the volume, and `mkfs` uses only features the oldest supported release
reads (ROADMAP §22.1).
...
| Checksums | CRC-32C (Castagnoli) over every metadata block, and over every data block on its own: one checksum per block of file data, never one per extent (NOCOW files and unwritten ranges have none; see those rows). Data checksums live in a checksum tree keyed by physical block number. A read loads the checksums of the blocks it reads before it submits them, so verification at completion reads nothing (DESIGN §10.1) | a hardware instruction on both architectures (SSE4.2 `crc32`, the Armv8 CRC32C instructions), and what ext4, XFS, and btrfs use; v1's CRC-32/ISO-HDLC has no instruction on x86_64. Per block, a read verifies only the blocks it reads, a write or truncate that splits an extent keeps its neighbours' checksums, and §29.4's scrub repairs one block at a time. A checksum over a whole extent would make a 4 KiB read of a 1 GiB extent read the whole extent, and every split recompute checksums over data it did not touch (F063 is v1's instance). Keyed by physical block, as btrfs keys its checksum tree, a block that snapshots and clones share has one checksum, and extent records stay fixed-size |
| Corrupt data | a data block enters the page cache only after the checksum that covers it verifies. A fill that fails leaves the page not up to date, and the page is never written back: `read` returns `EIO`, and a load or store through a mapping of the page raises `SIGBUS`. A write that changes part of a checksummed block, a partial `write` or a store to one page of a larger block, fills and verifies the whole block first, so it returns `EIO` and dirties nothing when the block fails. `fsck` and ROADMAP §29.4's scrub report the block | writeback computes a fresh checksum over whatever the page holds, so new bytes merged into an unverified page store the corruption under a valid checksum that every later read, `fsck`, and scrub accept. F063 was v1's instance, inside `Vol::write` (§9); the page cache moves the same merge into the fill (DESIGN §10.6) |
...
| Block size | a superblock field: `mkfs` writes 4096, and readers accept every power of two from 4096 to 65536; the superblock's own place does not depend on it (Superblocks and geometry) | a later default needs no format change |
| Superblocks and geometry | two 4 KiB superblock slots at byte offsets 65,536 and 1,048,576 from the start of the volume, whatever the block size. Nothing is written below byte 65,536, the blocks that hold that range and the two slots are never allocated, and a volume is at least 2 MiB. Commits alternate slots, as in v1 (§10). Mount validates both slots and takes the newer one whose tree roots pass their pointer checks (Verified pointers), otherwise the older. A fallback past a superblock whose checksum verifies is logged at error level with both generations, the next superblock records the rejected generation for `fsck-vibefs` to report, and that commit takes a generation above the rejected one; it applies no intent-log record that extends the rejected generation (Commit row). Each free extent the allocation map records carries the generation of the commit that freed it, and the allocator takes a block only when that generation is below the newest durable superblock's: a block a commit frees is reused only after the next commit's superblock is durable, across remounts too, a commit that frees nothing writes no allocation block to record the deferral, and the free space `statfs` reports leaves the held-back blocks out. `nblocks` is a superblock field that any commit may raise, online or offline; lowering it needs every block past the new end moved first. Free space and every other allocator structure are trees reached from the superblock, none with a place or a size that `nblocks` fixes at `mkfs`, and no superblock copy sits at the volume's end | a reader must find the superblock before it knows the block size, so the offsets cannot depend on it. The first 64 KiB is where boot sectors, protective MBRs, and partition tables live, and btrfs leaves it free for them (F003 stamped a GPT over a whole-disk vibefs image; DESIGN §10.5); the second slot, 1 MiB in, survives an overwrite of a disk's first megabyte. Without this one-commit deferral the next commit reuses blocks the older slot's tree names, so a lost write of the newest roots, or a bad sector in one, would leave nothing whole to mount; ZFS defers reuse for the same reason, and the cost is one commit's frees held back. A generation on each free extent keeps the deferral across a remount with no write of its own. A silent fallback would drop a commit that `fsync` may have acknowledged, so it is logged and recorded, as btrfs's `usebackuproot` and ZFS's rewind make the loss explicit, and a generation above the rejected one keeps two valid superblocks from ever naming two trees with one generation. ROADMAP §26.2 and §29.5 grow volumes in place, which a free-space map sized at `mkfs`, or a superblock copy at the end, would turn into a relocation |
| Extension | three feature-flag sets: compat (an older reader ignores the feature), ro_compat (an older reader mounts read-only), and incompat (an older reader refuses to mount); `VERSION` changes only for a change the flags cannot express; a release sets an incompat or ro_compat bit on an existing volume only by an admin command (§14) | additive changes land without a version bump, as in ext4 and XFS; v1 has one flag byte |
| Validation | every check §5 lists (entry counts, pointers in range, extent bounds, inline sizes, unique inode numbers, each metadata block referenced once within a tree, since snapshots share subtrees), with the allocation map in place of v1's refcounts, so that a reachable block the map marks free is `Corrupt`, runs when a block is read, before its contents are used. A metadata block referenced twice within one tree is the one check a single read cannot make, since each reference passes its own checks while the block is shared: `fsck` reports it (§11), and once a commit frees the block through one reference, a read through the other fails the allocation-map check, or, after the block is reused, the pointer check (Verified pointers row) | F061: v1's mount trusts on-disk counts, and a crafted image must return `Corrupt`, never panic (DESIGN §2.10) |
| Directories | an on-disk B-tree per directory with no volume-wide entry cap; a lookup reads one block per level. Each entry has a position cookie, an integer from 0 to 2^63 − 1 that is never reused within its directory: 0 is the directory's start, 1 and 2 belong to `.` and `..`, which the VFS synthesizes, and 2^63 − 1 is its end. A reader resumes from any cookie with one descent of a tree and then sees, once each, every entry that stayed in the directory, whatever was added or removed meanwhile | ROADMAP §14.8 (F067); `getdents64`'s `d_off`, `seekdir`, and NFS readdir cookies need stable positions, and `d_off` and `lseek`'s offset are signed, so a cookie at or above 2^63 cannot be passed back. A hash of the name (ext4's htree) collides, which makes positions ambiguous and costs ext4 a 32-bit and a 64-bit hash mode. btrfs meets this rule with a second index per directory, keyed by a sequence number never reused (its `DIR_INDEX`), at a second item per entry |
| File kinds | regular file, directory, symlink, character device, block device, FIFO, and socket; a device inode stores `rdev` as a 32-bit major and a 32-bit minor | v1 has the first three (§7). ROADMAP §13.2 creates FIFOs with §13.9's `mknodat`, which also creates device nodes, and §14.9's `mkfs-vibefs --from` copies device nodes and FIFOs from a tarball; the widths are statx's `stx_rdev_major` and `stx_rdev_minor` |
| Link counts | a 32-bit `nlink`; a directory's is 2 plus the number of its subdirectories, as ext4 and XFS report it | `find` and `fts` skip `stat` calls by counting subdirectories from a directory's `nlink`; v1 stores 1 for the root (§11), and btrfs's constant 1 turns that shortcut off for no saving |
| Orphans | a tree keyed by inode number holds every inode whose `nlink` reached 0 while an open file description still holds it, and every `O_TMPFILE` inode. The commit that drops the last name, or creates the `O_TMPFILE` inode, adds it; the first commit after its last close frees it with its blocks; `linkat` of an `O_TMPFILE` inode takes it off; and the first read-write mount, after any intent-log replay, frees every inode left on it | under copy-on-write, a commit taken while such a file is open records an inode that no directory names, and a crash would leak it and its blocks until a full `fsck` walk; ext4, XFS, and btrfs keep the same list. ROADMAP §17.1 relies on `O_TMPFILE`. Added after 1.0, it would need at least ro_compat, since an older writer that ignored it could free the same blocks twice |
| Inode flags | the 32-bit set that `FS_IOC_GETFLAGS` reports, at the bit values Linux's `include/uapi/linux/fs.h` defines (immutable, append-only, nodump, noatime, NOCOW, project-inherit, and the rest), with every bit v2 does not define reserved as zero; and a 32-bit project id | ROADMAP §23.1's `chattr` and `lsattr`, §29.5's per-project quotas through `FS_IOC_FSSETXATTR`, and the NOCOW files row; fields added after 1.0 would change every volume's inode record |
| Extents | an extent can be marked unwritten, for a range `fallocate` reserved: a read of it returns zeros, it has no data checksum until it is written, and a write converts it; a hole is a range no extent covers | ROADMAP §23.1's `fallocate` with `FALLOC_FL_KEEP_SIZE` and `FALLOC_FL_PUNCH_HOLE`, and `lseek`'s `SEEK_DATA` and `SEEK_HOLE`; ext4 and XFS keep the same flag, and without it a reservation would have to write zeros |
