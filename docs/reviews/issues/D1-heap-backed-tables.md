...
4. **Wake inbox:** the cross-CPU wake inbox (`src/ipi.rs:11-18`, `inbox_push` at `src/ipi_init.rs:174`) is a `u64` bitset indexed by `ThreadId`, so ids of 64 or more are dropped silently once `MAX_THREADS` goes. Replace it with a per-CPU bitmap of `[AtomicU64; threads/64]` sized from `Limits`, or with a lock-free intrusive MPSC list. Update the DESIGN §7 inbox text in the same commit.
