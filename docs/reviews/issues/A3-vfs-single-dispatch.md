...
3. **Trait completeness.** Add `readlink`, `rename`, and `sync` to `InodeOps` (vibefs already has `readlink`/`rename`; FAT has `rename`); default `readlink` returns `Inval` (`EINVAL`), as Linux's `readlink` does for anything but a symlink, and each other default returns Linux's errno for the missing operation (ROADMAP §10.4).
