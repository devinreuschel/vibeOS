...
1. **Crate:** `cargo fuzz init` inside `tests/fuzz` with `vibeos-hostlib-tests = { path = "../hostlib" }` (or `vibeos-core` after A2 step 5). Targets in `tests/fuzz/fuzz_targets/`:
   ...
   - `elf_parse.rs`: `elf::parse(data)`, seeded from the valid and truncated images `elf.rs`'s tests build.
...
- Eight targets build; each runs 60 s locally without a crash (or with crashes turned into regression tests).
