...
**Status (2026-09-22).** C1 landed (#77). DOC1+DOC3+R1 landed (#78). Harness CI flakes (#75), **B1+DX1** (#76), **Q1** (#80), and **T2+C2** (#79) on main. **B3** and **DOC4** implemented (version/changelog/release workflow; do not push `v0.8.0` from the agent). **Q4+E3+P2** implemented (feature names, `marker!`, Phase 17 lock notes). **E1** implemented (#86; portable-crate unwrap/expect/panic deny; `indexing_slicing` stays warn). **B2** implemented (#84): built-in `x86_64-unknown-none`, custom JSON gone. **O1 superseded** — Phase 9 exit closed 2026-09-22; do not add a "Phase 9 resumed" note. **S1** landed (SMEP/SMAP/UMIP/WP + virtio-rng/RDRAND `/dev/random`). Everything else still proposed.
