...
4. **Collapse build and harness duplication (B1, B2, T2, P1).** The Makefile carries five near-identical ISO recipes and five two-pass builds across five target directories, each of which rebuilds `core`/`alloc` because of the custom target JSON. Three Python drivers each construct their own QEMU command line and panic-signature list. A parametrized recipe, a single QEMU launcher, and a spike on the built-in `x86_64-unknown-none` target would cut build time and remove the "hostlib inherits a second `core`" hack. (CI wall time is already only a few minutes, so this is about simplicity and local iteration.)
...
| Target | Custom JSON target `x86_64-unknown-none-executable.json`, `-Zbuild-std`, `-Zjson-target-spec` | `Makefile:12`, `.cargo/config.toml` |
...
- **Observation:** `rust-toolchain.toml` says `channel = "nightly"` with no date, and CI uses `dtolnay/rust-toolchain@nightly`, so every build resolves to whatever nightly exists that morning; the project uses `#![feature(abi_x86_interrupt)]`, `alloc_error_handler`, `-Zbuild-std`, and `-Zjson-target-spec`, all of which have changed behaviour across nightlies. `setup.sh` clones `limine` at tag `v9.6.7-binary` with no hash check. Workflows reference `actions/checkout@v4`, `actions/cache@v4` by mutable major tag. The CI cache key hashes `**/Cargo.toml`, not `Cargo.lock`.
...
- **Observation:** `make test` builds the kernel in five `CARGO_TARGET_DIR`s (`target`, `target-panic`, `target-gp`, `target-kernel-tests`, `target-vibefs-crash`), each twice for the ksyms two-pass, and each rebuilding `core`, `alloc`, and `compiler_builtins` from source because the custom target needs `-Zbuild-std`. That is ten kernel links and five standard-library builds per full test run, before any QEMU time. CI caches `target` only.
- **Recommendation:** Share the `build-std` artifacts across the five builds (they differ only by feature set; a single target dir with distinct `--artifact-dir`/profile names keeps the "cannot leak into production" property while sharing std), or remove `build-std` altogether (B2). Cache all target dirs in CI.
...
- **Observation:** `x86_64-unknown-none-executable.json` is byte-for-byte equivalent to `rustc --print target-spec-json --target x86_64-unknown-none` in `code-model`, `features`, `disable-redzone`, `panic-strategy`, `linker`, `rustc-abi`, `max-atomic-width`, and `stack-probes`. The differences (`frame-pointer: always`, static relocation model, `pre-link-args` for `linker.ld`) are all expressible as rustflags (`-C force-frame-pointers=yes -C relocation-model=static -C link-arg=-T…`). The custom JSON is the sole reason for `-Zjson-target-spec`, `-Zbuild-std`, the `Makefile`/`.cargo/config.toml` split ("build-std here makes `tests/hostlib` compile a second `core`"), and much of P1. The maintainer confirms (2026-09-22) that the custom target was the agents' choice, not a requirement.
- **Recommendation:** Proceed. A one-day spike: build with `--target x86_64-unknown-none` plus the rustflags, diff `make layout` output and boot both firmware paths. If green, delete the JSON, the two `-Z` flags, and the config-split note; then hostlib can be a normal workspace member (A2).
...
- **Build and config:** `Cargo.toml`, `Cargo.lock`, `rust-toolchain.toml`, `.cargo/config.toml`, `x86_64-unknown-none-executable.json`, `linker.ld`, `limine.conf`, `build.rs`, `Makefile` (both the version at session start and the on-disk version after HEAD moved to `dd04ac9`), `setup.sh`, `.gitignore`, `.github/workflows/{ci,smp-stress}.yml`, `.cursor/environment.json`, `.cursor/rules/general.mdc`.
