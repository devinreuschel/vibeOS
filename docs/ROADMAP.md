...
- [ ] the panic path, backtrace, and symbol table working on aarch64: a frame-pointer walk along the `x29` chain that follows `x29` only into aarch64's known stacks, listed beside §10.7's x86_64 list: the current thread's KVA stack and the recorded boot stack; host tests cover both lists
