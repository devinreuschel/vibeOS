...
- [ ] every `SpinMutex`, `BlockingMutex`, and `RwLock` gets a lock class keyed by its initialization site, beside its DESIGN §2.1 rank, so every pipe's lock is one class; nesting two locks of one class takes an explicit subclass and a fixed order, such as address order for a socket pair or a two-directory `rename`; the classes carry DESIGN §2.1's two tiers, so a sleeping lock taken with a spinlock held, and a filesystem namespace or inode lock taken with the address-space lock held, are reported the first time they happen
