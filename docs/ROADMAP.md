...
- [ ] `close_range`, `copy_file_range`, `statx`, `faccessat2`, `openat2`, and `epoll_pwait2`, which glibc, coreutils, libuv, and Rust's `std` call before any fallback; on vibefs, `copy_file_range` copies bytes until the clone feature in [Beyond](#beyond) (VIBEFS.md §15)
