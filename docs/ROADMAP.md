...
- [ ] §12.5's error handling through stacked devices: a stacked device has no deadline of its own (DESIGN §10.3); when a member request completes with an error, its member's timeout and recovery included, the stacked device fails over to another path (NVMe multipath below) or fails that member of the array (§29.2), never the array, and a stacked request completes within the stacked device's stall bound, the sum over the members it can try in turn
