...
- [ ] vibefs and FAT images from each release mounted, read, written, and checked with `fsck` on `main`; and volumes that `mkfs` on `main` writes with its default features mount, read, write, and pass `fsck` on each supported release
