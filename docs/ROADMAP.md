...
- [ ] reverse mapping from a frame to the PTEs referencing it, which reclaiming a mapped page-cache page requires (§12.6) and swap reuses; a walk takes each address space's page-table spinlock, never its address-space lock (DESIGN §4.4); it takes no count on the space, holding the reverse-map lock for reading while it takes each region's page-table lock, and teardown unlinks a region under that lock taken for writing before it frees the region's page tables (DESIGN §2.11)
...
- [ ] per process: `statm`, `smaps` (which pins the address space as §13.9's `maps` does), `limits`, `io`, `mounts`, `mountinfo`, `fdinfo/`, `cgroup`, which reports the root group (`0::/`) until §21.5's cgroup v2, and `oom_score_adj`, stored and read back as on Linux, which §19.10's OOM score uses; `task/<tid>/` carries each per-pid file for its thread
