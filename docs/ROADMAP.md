...
describes the divergence. vibeOS takes nothing in a space Linux allocates as it goes (syscall numbers,
option, flag, and auxv values, device numbers, and the names of `/proc`, `/sys`, and debugfs files,
netlink families, and command-line options), since whatever it took would change meaning under software
built for a later Linux. A native interface lives under a vibeOS name, `/proc/vibeos/`,
`/proc/<pid>/vibeos/`, `/sys/kernel/vibeos/`, a `vibeos/` directory inside one Linux owns, a
`vibeos_<name>` generic-netlink family, a node under `/dev/vibeos/`, or a `vibeos.<name>=` command-line
option, and never takes a syscall number (SYSCALL.md §8).
...
- [ ] a stable interface changes only by an addition an older reader ignores, by a new version served beside the old one, or by a major version under the §22.1 policy
