...
- [ ] `/proc/sys`: `kernel/{domainname,threads-max,printk,random/uuid,core_pattern,dmesg_restrict}`, `vm/max_map_count`, `fs/{file-max,nr_open,inotify/*}`, and `net/core/somaxconn` beside §14.9's entries, all writable by root where Linux allows it, so `sysctl -w` works; `core_pattern` holds the pattern §13.8's command-line option sets at boot, and `dmesg_restrict` the §13.9 rule, both writable only by root
