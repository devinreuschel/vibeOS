...
- [ ] delivery builds a Linux `rt_sigframe` on the user stack (`siginfo`, `ucontext` with `uc_mcontext`, and the FP state), runs the handler, and returns through `rt_sigreturn`; the `ucontext` layout per architecture is pinned by a host test, since musl reads it; delivery copies the interrupted context from the thread's user frame (DESIGN §5.10) into the `ucontext` and rewrites the frame to enter the handler, and `rt_sigreturn` writes the saved context back into it, RCX and R11 included, so the return to the interrupted code takes `iretq` whenever RCX or R11 differs from RIP or RFLAGS
