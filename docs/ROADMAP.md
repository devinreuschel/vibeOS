...
- [ ] file capabilities, which Linux's `execve` rules above read: the `security.capability` extended attribute in Linux's `vfs_ns_cap_data` layout (revision 2, and revision 3 with its root uid once §21.5's user namespaces land) on vibefs v2 and tmpfs, settable only with `CAP_SETFCAP` and cleared by a write, as Linux clears it; unmodified `setcap` and `getcap` from the §14.9 mirror set and read it, and an in-guest test runs a binary carrying `cap_net_raw+ep` as uid 1000 and opens a raw socket
