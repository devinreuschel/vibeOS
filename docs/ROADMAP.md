...
the baseline forward to a later longterm series, in an edit that lists in `docs/LINUX.md`'s baseline log
every interface the move changes; from `v1.0.0` on a move ships as a minor release (§39.1). A native
interface or a deliberate divergence is listed with its reason in `docs/LINUX.md`, whose tables
`scripts/check_linux_md.py` checks, or has a line in this file naming the phase that replaces it, which
SYSCALL.md cites where it describes the divergence. vibeOS takes nothing in a space Linux allocates as
it goes (syscall numbers, option, flag, and auxv values, device numbers, and the names of `/proc`,
`/sys`, and debugfs files, netlink families, and command-line options), since whatever it took would
change meaning under software built for a later Linux. A native interface lives under a vibeOS name,
`/proc/vibeos/`, `/proc/<pid>/vibeos/`, `/sys/kernel/vibeos/`, a `vibeos/` directory inside one Linux
owns, a `vibeos_<name>` generic-netlink family, a node under `/dev/vibeos/`, or a `vibeos.<name>=`
command-line option, and never takes a syscall number (SYSCALL.md §8). Where vibeOS implements part of a
Linux interface that has its own discovery (a capability query such as `KVM_CHECK_EXTENSION` or
`VFIO_CHECK_EXTENSION`, a supported list such as `KVM_GET_SUPPORTED_CPUID`, or a feature bit), discovery
reports exactly the part implemented, as Linux reports a part it lacks, and `docs/LINUX.md` lists that
...
- [x] `switch_context` saves no FPU or SSE state; since §9.1, `syscall_init::on_switch` saves and restores each TCB's FXSAVE area (`switch_fpu`) before calling it; §10.6's FP binding leaves it the save alone and moves the load to the return to ring 3
...
- [ ] the kernel log of DESIGN §2.5's log contract: one lockless, sequence-numbered ring that any context may append to, NMI included, and a printer thread per console (lands in §19.5; today one global IRQ-safe ring and a serial try-lock sink); whole-line serial output lands earlier, in §10.2 (F138)
...
- [x] allocate vectors from the dynamic pool, bound to a chosen CPU; §11.3 moves the pool inside the x86 `IrqChip`, and drivers name an `IrqId` (DESIGN §5.4)
...
- [x] `Superblock` per mount, with a mount table and mount point crossing; §10.4's A3 box separates a mount from its superblock, so one superblock can back several mounts (DESIGN §2.11)
...
- [ ] crash consistency testing by killing QEMU at randomized points during a write workload on §10.2's volatile-cache device, then checking with `fsck` the images rebuilt from its trace. Reopened by the kernel review (F014, F080); lands in §10.2 and §10.11.
...
- [x] x87 and SSE state saved and restored eagerly with `fxsave64` and `fxrstor64` at syscall entry and exit and on every context switch (`switch_fpu`), which §10.6's FP binding replaces with a save at the switch away and a load before the return to ring 3, with no `xsave` and no lazy switching; the kernel itself stays soft-float on the built-in `x86_64-unknown-none` target (B2)
...
- [ ] a syscall count per process, summed over its threads and reported through syscall 500 until §13.9 moves it to `/proc/<pid>/vibeos/syscalls`; today the per-TCB `syscall_count` and the global `SYSCALLS` are incremented on every syscall and never read. Reopened by the kernel review (F150); lands in §10.7.
...
- [ ] `FS_BASE` saved and restored per thread on every context switch, and `fork` gives the child the parent's saved value, shown by an in-guest test that runs a `PT_TLS` binary across another process's exit; today `on_switch` does not switch it, so a TLS process resumes with whatever base its CPU last loaded, 0 after any process on that CPU exits or is killed. Reopened by the kernel review (F022); lands in §11.6.
...
- [ ] `make test-vibefs-crash` passes over the §10.2 volatile-cache device, and fails against a `vibefs_crash` build with a planted leak of one block per commit and against one that writes the superblock before the Flush ahead of it completes, both of which this line's §10.9 gate-map entry builds and runs (§10.2, §10.11)
...
- [ ] DESIGN §2.5's rule that nothing is silently swallowed has a check: clippy's `let_underscore_must_use` and `unused_result_ok` are denied in `[workspace.lints.clippy]`, so they reach `vibeos-core`, hostlib, and the kernel binary's clippy runs above (whichever of this box and the `undocumented_unsafe_blocks` box below lands first creates the table), and `#[cfg(test)]` code and the `kernel_tests`-only `ktest` module are exempt at their root. In the same PR every site the lints report becomes a returned error, a counter with a log line at most once a second, an error state a later call reports, or a retry with a named bound, or keeps its discard under `#[expect(clippy::let_underscore_must_use, reason = "...")]` whose reason names the case of DESIGN §2.5 that allows it; `marker!`'s `writeln!` into `Serial` keeps its discard under one `#[expect]` in the macro body; `log_init`'s two uncounted drops (the try-lock sink's copy and a record emitted during `log_fmt` re-entry) get a counter the panic dump prints beside the wrap count. `KError` and every module error enum are `#[must_use]`. If the audit is too large for one PR it splits by subsystem, a module not yet audited carries `#![expect(clippy::let_underscore_must_use, reason = "audit pending, ROADMAP §10.1")]`, and the box closes when none remains. DESIGN §2.5 and §2.7 describe the check as built in the same commit
...
- [ ] a kernel command line: `cmdline:` in the `limine.conf` entry, captured in `BootInfo` through Limine's executable command line request, then an `opt/vibeos/cmdline` fw_cfg file appended when QEMU's fw_cfg is present (probed on x86_64 only when CPUID reports a hypervisor, so bare metal never sees a write to port `0x510`), so the harness sets options on the unmodified ISO; parsed in the portable half with host tests; DESIGN §3.2 lists every option, and later phases add theirs to the same parser; option names follow DESIGN §3.2: Linux's name and meaning for an option Linux defines, `vibeos.<name>=` for one only vibeOS defines, and `sysctl.<path>=` for a sysctl vibeOS implements, with any other path logged and ignored; a word the parser does not recognize reaches init as Linux passes it (an undotted `name=value` into its environment, another undotted word or any word after `--` as an argument, a dotted word dropped), and host tests cover each rule
- [ ] `vibeos.ktest=<glob>` runs only the matching in-guest tests and `vibeos.ktest_repeat=<n>` runs them `n` times in one boot (a test that cannot run twice says so in the registry and runs once), set by `VIBEOS_KTEST` and `VIBEOS_KTEST_REPEAT`, with the harness reporting passes out of runs and multiplying its boot timeout by the repeat count; `loglevel=` sets the §5.5 runtime level at boot
...
- [ ] a volatile-cache device, the backend of `make test-vibefs-crash`: an NBD server in hostlib that advertises flush, so QEMU forwards every guest flush, serves the guest's virtio-blk disk with `write-cache=on` and records in a trace every write and flush it receives and every reply it sends. It replies to a write when the write arrives and to a flush only after a delay the run's seed picks, reading and recording the requests that arrive meanwhile, so a write the guest sends before a flush completes is not covered by it; a write is durable once the device has completed a flush it received after replying to that write. Killing QEMU loses no write the host has received, with the harness's `cache=writeback` and equally with the `cache=none` or `writethrough` that KERNEL_REVIEW.md §8.2 suggests, so only a rebuild from the trace can show a lost unflushed write. After each kill the harness rebuilds images from the trace: for every superblock write, the writes durable when it arrived plus that write alone; and at the kill, the durable writes plus a seeded random subset of the later ones. The server and QEMU's NBD client run on the Linux and macOS hosted runners, and `make test-vibefs-crash` moves onto the device in the commit that lands §10.11's flush box, since until then a Flush can reach the device ahead of the writes it should cover (F080)
- [ ] the vibefs crash test can fail over the volatile-cache device above. The guest prints `wr N` before each commit; on any `sync_fs` error it prints a registered failure line and halts. Each round kills QEMU at a random commit count between 1 and 200 and fails unless the harness killed QEMU itself. On every image the harness rebuilds, `fsck-vibefs` must report `errors 0 warnings 0`, and the harness reads `/crash/w` from it with a hostlib tool over `vibeos-core`'s vibefs and requires the content of one iteration, no older than the last whose commit's final flush the device completed before that image's crash point; for the image at the kill, that is iteration N or N-1 for the last `wr N` it saw. This box lands with §10.11's commit-leak fixes (F080)
...
- [ ] the host test `switch_context_roundtrip` runs the kernel's switch asm on x86_64 hosts, from `tests/hostlib`, which includes the x86_64 port's switch assembly when the host is x86_64, since `vibeos-core` carries no assembly (the Phase 10 gate), with `cli` and `sti` supplied by a macro, in place of the `#[cfg(test)]` copy in `thread.rs`, which has neither and which aarch64 hosts compile out (F142)
...
- [ ] an `arch` module boundary built from DESIGN §11.1's seam table, which this box cites instead of listing concerns: one trait for each row whose Seam is a trait; the x86_64 port's pure half (its descriptor encodings, vector constants, and 8259 and APIC encodings) in `vibeos-core` under `arch/x86_64/`, compiled on every host with no `cfg`, beside the stub port; its hardware half implementing the traits on one zero-sized type; and portable code taking the port as one type parameter, bounded by the traits it calls or by the umbrella trait `Port`, never `dyn` (DESIGN §11.1)
- [ ] every x86 assumption outside `arch/` found by grep for `asm!`, `x86`, and CR and MSR names, then moved or fenced; the audit checked in as `docs/ARCH.md`, which maps each row of DESIGN §11.1's seam table to its implementing modules
...
- [ ] `current` is read in one instruction that preemption cannot split (DESIGN §2.9 rule 5): `arch::current_tcb()` is one `gs`-relative load of `PerCpu.current`, and `per_cpu_init::current_thread`, `thread_init::current_id`, and `current_pid` become wrappers over it; `arch::cpu_id_hint()`, one `gs`-relative load of `PerCpu.cpu_id` that returns the value, serves the callers that tolerate a stale id (the log prefix, virtio-blk's `prefer_vq`, the panic dump); `per_cpu_init::current()`, `try_current()`, and the `per_cpu!` macro assert IF=0 in debug builds from `irq: enabled` on, so every other per-CPU access, `log_init`'s `EMITTING` and `STAGE` slot index included, moves inside an IF=0 stretch. `scripts/check_current.py` in `make check` fails on a read of `PerCpu.current` outside `src/arch/`, and `tests/harness/test_check_current.py` covers it. It lands after the F039 box above. The race proof is deterministic, since TCG takes an interrupt only between translation blocks: the in-guest test `current_at_if1` reads `current_pid()` and `current_id()` from a kernel thread with IF=1, which trips the IF=0 assertion on the two-step accessors and passes on `current_tcb()`. In `make test-kernel-smp4`, with the `kernel_tests` hook of §10.6's FP binding box that requeues each preempted thread on the next online CPU (whichever of the two boxes lands first builds it), four threads read `arch::current_tcb()` and `current_pid()` with IF=1 for 2 s and fail on any value not their own (F039)
...
- [ ] a pid is not a table index (DESIGN §2.11 rule 4): pids and tids come from one allocator, a process's pid being its first thread's tid, in increasing order up to `pid_max` (Linux's default, 32,768, until §23.4 makes it `kernel/pid_max`), wrap to 300, Linux's `RESERVED_PIDS`, skipping ids in use, and map to their table slot through a lookup, so a reaped pid is not reused until the counter comes round. Today `proc_init::alloc_pid` hands out the lowest free slot index, so the next `fork` reuses the pid of the process just reaped, and a stale `kill` or `wait4` reaches the new process (F127). A host test allocates and frees 100 pids and sees none reused before the wrap; another marks an id as still carried by a process group after its process is reaped, and the allocator skips it at the wrap; a third, with `pid_max` at 400, allocates past it and finds the first id after the wrap is 300
- [ ] `MAX_ELF` removed: the loader maps segments from the file instead of reading the whole binary into a bounded buffer
- [ ] allocation after `irq: enabled` is fallible on every path (DESIGN §4.4): `vibeos-core` gains `kalloc` (`TryBox`, `TryVec`, `TryString`, `TryArc`, and an ordered map), whose growing operations return `Result` and which builds on stable Rust; `clippy.toml`'s `disallowed-types` denies `alloc`'s `Box`, `Vec`, `String`, `Arc`, `Rc`, and `alloc::collections` types in both crates outside `kalloc`, and `disallowed-macros` denies `vec!` and `format!`; each boot-time site that keeps an `alloc` type runs before `irq: enabled` and carries an `#[allow]` whose comment names its boot step, and every other allocation is fallible, its failure returning `ENOMEM` or, on a kernel thread, handled as DESIGN §4.4 says; host tests fail each `kalloc` operation through a counting allocator and find the collection unchanged. A `kernel_tests` hook fails every heap allocation after a chosen count; armed at every count from zero to the number of allocations each call makes, `fork`, `execve`, and `open` from a user program each return `ENOMEM` and the kernel stays up, and a failed `execve` returns to the caller's old image, as DESIGN §4.4's point-of-no-return rule requires. Armed at zero across a child's `exit` and its parent's `wait4`, the parent gets the child's status, the hook sees no allocation from the child after its exit's first release or from the `wait4`, and the kernel stays up. An in-guest test registers a `kernel_tests` driver whose probe allocates, for a PCI device no other driver binds, as `test_pci_bind_order` registers one, and binds it with the hook armed at zero: the probe fails, the device stays unbound with a log line naming it, and the kernel stays up (F010)
...
- [ ] FAT and vibefs behind `InodeOps`; `Vfs` owns inodes, dentries, mounts, and files and nothing backend-specific; FAT's `Vfs` inode is the F013 inode below, moved into `Vfs` and not copied: its key is the dirent location, the first cluster and dirent slot are backend-private words that only FAT reads and writes under its volume lock, and the size is the `Vfs` inode's size. The F013 table (`FatVol`'s `inos`, with `ino_of`, `put_size`, and `by_ino`) is deleted in the same PR, F013's two-descriptor host test and `fsck.fat -n` pass unchanged, and in-guest, through `Vfs`'s File API, two descriptors on one empty FAT file on the initrd, one of which writes 5,000 bytes, get one `st_ino` and a size of 5,000 from `fstat`, and `Vfs` holds one inode for the file; a mount and its superblock are separate counted objects (DESIGN §2.11): a `Mount` holds a counted reference to its `Super`, a second mount of a block device that is already mounted shares its `Super`, and one whose read-only flag differs returns `EBUSY`, as Linux's does; a dentry belongs to its superblock, and a mountpoint is found by its parent mount and dentry (A3)
...
- [ ] one refcounted in-core FAT inode per file, keyed by its dirent location, owns the first cluster, the size, and the dirent slot (it is `FatVol`'s `inos` table, which `ino_of` already keys by `(dir_clu, dir_off)`, extended with a count rather than built beside it; AGENTS.md rule 10), and an open file holds only a reference, its offset, and its flags; FAT read, write, and truncate update the inode under the volume lock, rename re-keys it, unlink unhashes it so a file created later in the freed dirent slot gets a new inode, and `free_chain` runs at the last close of an unlinked file; FAT and vibefs `O_APPEND` and `SEEK_END` read the inode's size; a host test writes through two descriptors on one empty file, truncates through one and extends through the other, and the host-test `fsck` helper (`fsck.fat -n`) passes. Its identity is the dirent location (`dir_clu` and `dir_off`; the root directory `(0, 0)`), never the first cluster, which changes on an empty file's first write and on truncate to 0; it is the inode §10.4's A3 box moves into `Vfs` (F013)
- [ ] the dentry cache never resolves through a reused slot: a directory dentry stays pinned while a child dentry names it as parent, `dcache_find` matches the superblock as well as the pinned parent and name, a mountpoint is found by its parent mount and dentry, so a volume that two mounts show has one dentry per name, `Vfs::mount` pins the mountpoint before `dentry_force_alloc`, and `Vfs::umount` makes every busy check before it changes state; host tests put `/a` under eviction pressure and still find the mount on `/a/m`, and find that `stat("/c/x")` never returns `/a/x` (F065)
...
- [ ] syscall dispatch indexes the generated syscall table by number, taken as Linux takes it (SYSCALL.md §1: `eax` sign-extended on x86_64, the low 32 bits of `x8` unsigned on aarch64), and converts each argument to the C type the row declares for it (the width and signedness of `int`, `unsigned int`, `long`, `size_t`, `pid_t`, or a pointer), so no handler sees a bit its type does not have. The dispatcher checks no pointer itself: each handler checks a pointer where it first copies through it, after the descriptor, flag, and existence checks Linux's handler makes first (SYSCALL.md §3). Each row declares its pointer arguments (a buffer with its length argument, a fixed-size value, a C string, or a string vector); the rows for `open` (path), `execve` (path, argv, envp), and `wait4` (status) declare theirs, where `syscall.rs` gives all three a `ptr_mask` of 0 today. The declarations drive the §10.7 trace, §13.13's fuzzer, and an in-guest test that passes an unmapped and a kernel-half pointer in each declared pointer argument, with every other argument valid, and gets `-EFAULT`, so a declaration cannot drift (F150). Host tests feed the dispatcher numbers and arguments with their high 32 bits set and find each handler sees what Linux's would; in-guest, `read(-1, <unmapped>, 1)` returns `-EBADF` and `wait4` with no child and an unmapped status pointer returns `-ECHILD`
...
- [ ] user-VA accessors replace the physmap copy for syscalls. `copy_from_user` and `copy_to_user` keep the range check (canonical, above the null guard, inside the user half, no overflow), because inside `stac`/`clac` the MMU still allows supervisor access to kernel pages. They then dereference the user address inside `stac`/`clac`, so `CR0.WP` faults a write to a read-only page and a not-present page faults. A fault inside an accessor becomes `EFAULT` through an exception-table fixup, which replaces the page-table pre-walk. Today `write_bytes` writes through the physmap and ignores `WRITABLE`, so `read`, the `wait4` status pointer, and `psinfo` can write into the caller's read-only and executable pages, which Phase 12's COW would turn into cross-process corruption. An accessor reports how many bytes it copied before a fault it cannot resolve; `read`, `write`, and the other calls that return a byte count return that count when it is above 0 and `EFAULT` only when it is 0, and a call that changed state before its copy-out keeps the change (`wait4` has reaped the child) and returns `EFAULT`, as Linux's do (SYSCALL.md §5). In-guest, a 256-byte `read` of a regular file into a buffer that starts 100 bytes below an unmapped page returns 100, and a `wait4` whose status pointer is unmapped returns `EFAULT` and a second `wait4` then returns `ECHILD`. DESIGN §5.1 and SYSCALL.md §5 are rewritten in the same commit to describe the accessors as built (F023)
...
- [ ] an IST vector taken at CPL 0 (NMI, `#MC`, `#DB`), and `#DF` at any CPL, decides whether to `swapgs` from the sign of `GS_BASE`, not from CS.RPL; one taken at CPL 3 swaps by CS.RPL and continues on the thread's kernel stack (DESIGN §5.10 rule 3; the F005 box below). The syscall entry before its `swapgs` and the exit's `swapgs; sysretq` and `swapgs; iretq` run at CPL 0 with the user GS base loaded. An in-guest test puts hardware execute breakpoints on those instructions, and the `#DB` handler finds its `PerCpu`. The sign check holds until §18.3 lets userspace write a kernel-half GS base (F007)
...
- [ ] `enter_user` and `enter_user_full` execute `cli` before they load the null selector into DS, ES, FS, and GS and write `GS_BASE` and `FS_BASE`, and `iretq` restores the user IF from the saved RFLAGS. Today a spawned or forked process reaches `enter_user_full` with IF=1, so an interrupt after `mov gs` reads `gs:[0]` at VA 0 at CPL 0 and halts the kernel. The selector loads, MSR writes, and `iretq` run as one asm block after the `cli`. An in-guest test forks and exits 1,000 children on CPU 0 while CPU 1 sends CPU 0 reschedule IPIs, and the kernel stays up (F006)
- [ ] ring 3 runs with Linux's x86_64 segment selectors (DESIGN §5.1): user data at GDT slot `0x28` and 64-bit user code at `0x30`, slots `0x18` and `0x20` null, the TSS at `0x38`, and `IA32_STAR`'s SYSRET base `0x23`, so `sysretq` loads SS `0x2b` and CS `0x33`, as Linux does; `src/desc.rs`'s constants and its host test pin `0x33`, `0x2b`, the TSS selector, the STAR value, and the SYSRET arithmetic, and DESIGN §7.2's `IA32_STAR` row and SYSCALL.md §1 describe them as built in the same commit. DS, ES, FS, and GS hold the null selector in ring 3: `execve` and a new process's first entry load it into all four, `fork` and `clone` copy the parent's four, and the context switch saves each thread's and loads the incoming thread's before it writes `FS_BASE` and `GS_BASE` (DESIGN §7.5, AGENTS.md rule 8). In-guest: a user program reads CS `0x33`, SS `0x2b`, and DS, ES, FS, and GS 0; the child of a parent that loaded `0x2b` into DS reads `0x2b`; and two processes on one CPU, one of which loaded `0x2b` into DS, yield to each other 1,000 times and each reads back its own DS every time
- [ ] every IDT vector enters through a stub that `arch/idt.rs` generates from one vector table, and the stub makes the `swapgs` decision and saves CR2 for `#PF` and DR6 for `#DB` into the frame before the body runs (DESIGN §5.10 rule 9), so the AC clear and the IST sign check above each live in one place. `idt::set_handler` takes a body function, not an `extern "x86-interrupt"` handler. The table has one row for each vector 0 to 255, and each row's error-code flag is `vectors::pushes_error_code(v)` evaluated in a `const` context, so no literal list of error-code vectors exists and `error_code_set_matches_sdm` tests the function the stubs use. Today the pool stubs `irq_init::device_irq::<N>` (vectors 0x31 to 0x7F) and `kbd_init`'s `kbd_ioapic` and `kbd_pic` skip `gs_enter`, so an interrupt through any of them at CPL 3 reads `gs:[0]` at VA 0 and halts the kernel. `scripts/check_entry.py` in `make check` fails on an `extern "x86-interrupt"` function outside `src/arch/`, and once no such function remains anywhere, `#![feature(abi_x86_interrupt)]` leaves `src/main.rs`. The in-guest test `user_device_irq` runs ring 3 with IF=1 on CPU 0 while CPU 1 sends it a pool vector and the keyboard vector, and `user_ipi` does the same with reschedule IPIs; in both the process and the kernel keep running (F004, F093)
- [ ] an interrupt no handler owns does not halt the kernel (DESIGN §5.2, §2.10): the vector table's default body for `0x20`–`0xFF` and `irq_init::dispatch`'s unowned case count the interrupt per vector and per CPU, EOI the controller that delivered it (the LAPIC when its in-service bit for the vector is set, else the 8259), log `irq: no handler for vector 0x<v> cpu <n>` with the running count at most once a second per vector, and return; the 8259 handler counts a spurious IRQ7 or IRQ15, which it already detects from the ISR, and masks at the PIC, EOIs, counts, and logs any other line no driver claims. It lands after the box above. Host tests cover the EOI choice and the 8259 decision for each of the 16 lines, spurious and real; in-guest, 10,000 self-IPIs each to an unallocated pool vector, to `0x85`, and to `0x25` with the 8259 masked leave the kernel up, each vector's count at 10,000, and at most one log line per vector per second; a count below 10,000 for any of the three shows a missed EOI or a lost delivery. §25.7's soak samples the per-vector counts, where a persistent storm shows as a rising count. DESIGN §2.10, §3.3, §5.2, §5.4, and §5.5 describe it as built in the same commit
- [ ] one user frame (DESIGN §5.10): the syscall entry, and every generated stub for a CPL-3 frame, saves the whole user frame at the top of the thread's kernel stack before it calls a body, in the order of the first 21 words of Linux's `user_regs_struct` (`orig_rax` the syscall number or -1, the tail the `iretq` frame), with RCX stored in both the `rcx` and `rip` slots and R11 in both `r11` and `rflags`; a host test pins the frame's field offsets. The syscall exit stores the return value in the `rax` slot and takes `sysretq` only when `rip == rcx`, `rflags == r11`, `cs` and `ss` are the user selectors, `rip < USER_MAP_END`, and RF, TF, and VM are clear; otherwise it restores all 15 GPRs from the frame and runs `iretq` on its tail, after the non-canonical check of the `USER_MAP_END` box above. `PerCpu.syscall_scratch` shrinks to the one word that holds the user RSP between `syscall` and the stack switch, so no exit path writes `gs:` scratch. `SyscallFrame` and `UserRegs` become this one type, which portable code reaches through the §10.3 user-context trait (the registers, the original syscall number, and the restart rewind), and a spawned or forked thread's first return to ring 3 is the ordinary exit over a frame its creator wrote, so `enter_user_full` and `vibeos_iret_user_full` go (AGENTS.md rule 10). It lands after the generated-stub box above and the syscall-exit and IF=1 boxes (F001, F011). Today the 16-word syscall frame has no RIP, RFLAGS, CS, SS, or syscall-number slot, the exit reads RIP and RFLAGS from the RCX and R11 slots and stages the `iretq` frame in `gs:` scratch, and an `x86-interrupt` handler saves only the registers it clobbers, so no context but the one `syscall` created can be returned to with RCX and R11 intact, and nothing records the user registers of a thread preempted in ring 3. In-guest, and on the §10.1 KVM leg: a `kernel_tests` hook sets a `getpid`'s saved `rcx` to a canary that differs from `rip`, and the program finds the canary in RCX after the call; and a user loop that holds canaries in all 15 GPRs runs on CPU 0 while CPU 1 sends it 1,000 reschedule IPIs, and a `kernel_tests` hook finds the canaries in the preempted thread's user frame each time. DESIGN §7.5 and SYSCALL.md §1 describe the frame as built in the same commit
- [ ] a pending signal whose default action terminates or stops the process takes effect on every return to ring 3: every exit to ring 3, the syscall exit included, checks the current process's pending set with IF=0 after its `cli` and loops when it finds work, as DESIGN §5.10 rule 11 requires, with a debug-build assertion of IF=0 at the check; handler delivery stays in §13.8. Today signals apply only at syscall entry and after the `wait4` sleep, so `SIGKILL` cannot end, and `SIGSTOP` cannot stop, a process that makes no syscalls. It lands after the stop-wait box above. An in-guest test kills one child looping in ring 3, stops and continues another, and reaps both. A `kernel_tests` hook, run right after the last check of a syscall exit, posts `SIGKILL` to the returning process and sends this CPU a reschedule IPI, and a `kernel_tests` record shows the kill acted on in the exit of that IPI's entry from ring 3; a check made with IF=1 lets the IPI be taken before the `cli` and leaves the kill to the next tick, which the record shows. SYSCALL.md §1 states the rule in the same commit (F033)
- [ ] no fault or trap that ring-3 code raises reaches `exception_halt`: the x86_64 port's pure half decodes each vector and error code into the portable `TrapKind`, one table in `vibeos-core` gives each `TrapKind` its CPL-3 action (the signal and the `si_code` Linux sends), `sig_for_vec` reads its answer from them, and a host test fails on a vector 0 to 31 without one (DESIGN §5.2). `debug_ex` calls `try_user_fault` for a CPL-3 frame after the stub has swapped GS by CS.RPL and moved the frame from the IST stack into the thread's user frame, and the frame returns through the common return to ring 3, exit work included (DESIGN §5.10 rule 3), as Linux's `idtentry_mce_db` does, since the kill path can block; the stub saves DR6 into the frame and clears it before the move, and the body reads DR6 only from the frame (DESIGN §5.10 rule 9). `sig_for_vec` maps `#DB` to `SIGTRAP` and `#AC` to `SIGBUS`, and CPU init sets `CR0.AM` on every CPU, as Linux's does, so a user program that sets `RFLAGS.AC` gets `SIGBUS` for a misaligned access; alignment checks apply only at CPL 3, so the accessors' `stac` is unaffected. Setting AM replaces the kernel review's F005 fix, which cleared it only to avoid the halt that the `SIGBUS` mapping removes. Today a user `popf` that sets TF, or an `int1` (`0xF1`), halts every CPU. In-guest, a child that sets TF with `popf` and a child that executes `int1` each end with `SIGTRAP` in their `wait4` status; a third child sets TF while a `kernel_tests` hook at the top of the CPL-3 `#DB` body yields, re-pins the thread to the other CPU, and clears TF in the saved frame, and the child then makes 100 `getpid` calls, at each of whose entries the hook finds `PerCpu.apic_id` equal to the running CPU's LAPIC ID, and exits 0; on the §10.1 KVM leg, since TCG does not model alignment checks, a child that sets AC with `popf` and loads four bytes from an odd address ends with `SIGBUS` in its `wait4` status; DESIGN §2.5 and §5.2 drop their F005 not-yet-enforced notes in the same commit (F005)
...
- [ ] one per-CPU control-register routine, run on the BSP and on every AP, sets `CR0.NE`, `CR0.MP`, and `CR0.AM`, clears `CR0.EM` and `CR0.TS`, and sets `CR4.MCE`, `CR4.OSFXSR`, `CR4.OSXMMEXCPT`, and `CR4.PGE`. Today no CPU sets NE, MCE, or OSXMMEXCPT, so a user x87 error raises the masked IRQ13 and a machine check shuts the CPU down, and only the AP trampoline sets PGE, so the BSP runs with PGE clear, as Limine's CR4 of `0x20` leaves it. The per-CPU CR in-guest test asserts each bit on every CPU. A user x87 divide by zero with the zero-divide exception unmasked ends with `SIGFPE`; on the §10.1 KVM leg, so does an SSE divide by zero with `MXCSR.ZM` clear. DESIGN §5.2's `#MF`, `#XF`, and `#MC` rows then hold (F026, F085)
- [ ] one FP binding on x86_64, the rule DESIGN §7.5 states for both architectures and §11.6 reuses: a per-CPU `fp_owner` and a per-thread `fp_cpu` say whose state the FP registers hold, and they hold thread T's only when this CPU's `fp_owner` is T and T's `fp_cpu` is this CPU. The switch away from a thread whose state is live saves it with `fxsave64` and loads nothing; every return to ring 3 (the syscall exit, the vector exit to CPL 3, and `enter_user`, and `enter_user_full` until the one-user-frame box above removes it) checks the binding with IF=0 and, when the registers hold another thread's state, loads `Tcb.fpu` with `fxrstor64` and sets both fields. The syscall entry's `fxsave64` and the exit's `fxrstor64` are deleted, `switch_fpu` stops loading, and `CR0.TS` stays clear, so no FP access traps. A new TCB starts with `fp_cpu` empty; code that writes a thread's `Tcb.fpu` empties its `fp_cpu`; code that reads the running thread's saves the live registers first, inside an `InterruptGuard`. The binding's transitions are a pure state machine in `vibeos-core`, with host tests for a migration, a reused TCB address, and a write to saved state. `make` runs `scripts/check_kernel_fp.py` on each linked kernel ELF: with the pinned toolchain's `llvm-objdump` (the `llvm-tools` component) it fails on any x87, MMX, SSE, or AVX instruction outside the FP save and load routines its allow-list names, since nothing traps a kernel FP use; `tests/harness/test_check_kernel_fp.py` covers it. DESIGN §3.1's target note, §7.5's FP row, and SYSCALL.md §1 describe the binding as built in the same commit. It lands after the syscall-exit box (F001) and the `enter_user` box (F006) above, and the F069 box below lands after it. In-guest, on one CPU: process P fills XMM0-15 with a pattern and yields in a loop while a new process Q starts, and no XMM register Q reads at its first instruction holds P's pattern; in `make test-kernel-smp4`, a `kernel_tests` hook requeues each preempted thread on the next online CPU, and a user thread that keeps a counter in `xmm0` and checks it against memory on every loop iteration runs for 2 s with no mismatch
- [ ] x86_64 user FP state follows the psABI across `fork` and `execve`, from one constant initial image: `vibeos-core` holds the psABI initial FXSAVE image as a `const` (FCW `0x037F` at bytes 0-1, MXCSR `0x1F80` at bytes 24-27, every other byte zero), and `execve`, `spawn_user`, kernel-thread creation, and `init_bootstrap` all start from it; `FPU_TEMPLATE`, `fpu_template()`, `init_fpu`'s `fninit` and `fxsave64` capture, and the `Fxsave::empty()` fallback are deleted. `sys_fork` saves the parent's live registers under the FP binding (the box above), then copies `Tcb.fpu` into the child before `make_ready`; `sys_execve`, after its point of no return, writes the constant image into `Tcb.fpu` and empties the thread's `fp_cpu`, so the return to ring 3 loads it and a switch before then saves nothing over it. SYSCALL.md §1, DESIGN §2.7's I25 row, and DESIGN §7.5 drop their F069 and F129 notes in the same commit. A host test pins the image bytes; `/bin/tests` sets MXCSR's rounding bits and `xmm0` before a `fork` and the child reads both back, and an image that `execve` starts reads MXCSR `0x1F80`, FCW `0x037F`, and `xmm0` zero; so does `/sbin/init` at its first instruction under `make test-e2e-uefi`, where OVMF may leave data in the XMM registers (F069, F129)
...
- [ ] the panic dump owns COM1 through DESIGN §2.5 step 1's stop primitive, which every later path that stops the other CPUs reuses: the first CPU into `begin_dump` claims the dump before it stops anyone, and a CPU that finds it claimed by another runs the stop routine without writing; the owner sets `HALTING`, sets STOP in each other online CPU's request word and sends it the `0xFE` IPI, waits up to 100 ms of TSC time for acknowledgements, sends NMI to each CPU that has not acknowledged, and waits 10 ms more; `service_incoming` reads the request word before any slot, so a CPU in a serviced spin stops without the IPI; the stop routine saves the interrupted registers and frame pointer in a per-CPU crash-register slot, sets a `stopped` flag, acknowledges, and halts, and the dump prints each slot; the NMI handler reads and clears its CPU's request word, halts at once on a CPU already stopped, and returns at once on the dump owner when no request is pending; A4's raw serial layer (§10.3) holds `HALTING`, the owner's CPU id (the `DUMPING` flag moves there), and a `write_owner` that takes no lock and no `InterruptGuard`, and once `HALTING` is set a serial write or log append on any other CPU runs the stop routine instead of writing; the dump prints a registered `vibeOS: panic: cpu N stopped (ipi|poll|nmi|panic)` or `vibeOS: panic: cpu N not stopped` line for each other online CPU. Today `halt_others` returns without waiting, a CPU spinning with IF=0 never takes the IPI and writes COM1 without the TX lock, and a second CPU into `begin_dump` re-runs `Serial::init` mid-dump. DESIGN §2.3, §2.5, §2.7 (I32), §5.2, §7.6, and §8.3 describe it as built in the same commit. A `panic_test` variant at `-smp 5` panics CPUs 0 and 1 at once while CPU 2 prints a numbered line in a loop with IF=0, CPU 3 waits with IF=0 on a `SpinMutex` that CPU 0 holds, and CPU 4 spins with IF=0 in a loop that neither polls nor writes, and the dump owner sends itself an NMI IPI after its panic message; the harness finds one panic message, its backtrace, one `vibeOS: panic: halted` line, no numbered line after the first `vibeOS: panic:` line, one of CPUs 0 and 1 `stopped (panic)`, `cpu 2 stopped (poll)`, `cpu 3 stopped (poll)`, and `cpu 4 stopped (nmi)`, and the panic exit that §10.2's expect-panic e2e requires (F135)
...
- [ ] syscall tracing can be switched on: `vibeos.strace=1` on the §10.2 command line calls `syscall_init::set_trace(true)`, which nothing calls today, and SYSCALL.md §6 names the option. An e2e boot with `vibeos.strace=1` finds a `user: syscall write` line for the first user `write` (F150)
...
kernel on a weakly ordered CPU. An ordering fix, whose reordering no stall hook can produce and TCG on
an x86_64 host never shows, is proved by its loom model's or litmus test's weakened variant (§10.8,
§11.7); the aarch64 tiers on the arm64 runner and §11.7's HVF records are the backstop for code no
model covers.
...
and the data checksums as it found them. The kernel writes to a disk only for a mounted filesystem or a write to its device node, and a `Flush`
covers every write whose completion was reported before it was submitted (DESIGN §10.2).
...
- [ ] the block layer keeps only the orders DESIGN §10.2 names: `Op::Barrier`, `block_init::barrier`, `virtio_blk_init::barrier`, `cache_init::barrier`, `cache::cached_barrier`, and the queue's fence logic are deleted, along with the `barrier()` calls in the ramdisk and virtio-blk read-write in-guest tests; a `Flush` goes to the device when it is submitted and holds up no later request; `cache_init::flush` waits for each write it issues and for the writes the writeback thread already has in flight, then sends its `Flush`; `cache_init::flush` waits through a WRITEBACK state on each block-cache slot: a slot whose write is in flight keeps its key and stays readable, is never evicted or re-keyed, and takes no second write to its LBA until the first completes, a waiter sleeps on the slot's wait queue, and `flush` waits for the device's WRITEBACK slots before it sends the Flush; a write can carry `Fua`, which the block layer completes on a device without FUA by waiting for the write and then sending a `Flush`; and `src/block.rs`'s module doc states this contract. The host tests `barrier_holds_later_requests`, `flush_is_fence_then_device_op`, `no_merge_across_barrier`, and `barrier_writes_dirty_no_device_flush` are replaced: a host test on the `Queue` finds a read submitted after a `Flush` dispatched before the `Flush` completes; a host test finds a `Fua` write on a device without FUA reported complete only after a `Flush` that was dispatched after the write completed; and an in-guest test holds one `blk-wb` write at a `kernel_tests` hook and finds `cache_init::flush` send no `Flush` until the hook releases it. Another host test holds a writeback in flight, dirties the page again, and fills the cache past its size, and finds the slot still keyed, one write to that LBA in flight, no Flush sent until that write completes, and the newer data read back after it. A device class vibeOS drives that gives a filesystem no completion to wait on (virtio-blk, NVMe, and AHCI each give one), or a device found to drop its volatile cache on a reset, which §10.2's volatile-cache device can model, reopens DESIGN §10.2's contract (F015, F043)
- [ ] the block queue keeps DESIGN §10.2's overlapping-write order: `pick()` never dispatches a write while an older write to an overlapping range is queued or in flight, and sequence numbers are `u64`. A host test on the `Queue` submits a write to LBA 100, a read at LBA 0, and a second write to LBA 100, and finds the read dispatched while the first write is in flight and the second write dispatched only after the first completes (F043)
- [ ] virtio-blk negotiates `F_RO`: a write to a read-only device fails at once with a new `BlockError::ReadOnly`, with no retry, and the device keeps serving reads. The `version1_required_and_no_ro` host test is inverted to expect `F_RO`, and `make test-kernel` gains a boot, limited by `vibeos.ktest=` to one test, whose `vda` is an unpartitioned `readonly=on` image: one write gets `ReadOnly` and a read after it succeeds (F046)
- [ ] a virtio-blk request that exhausts its 3 retries fails alone: `finish` calls `fail_rest` only when the device sets `DEVICE_NEEDS_RESET`, and DESIGN §10.3 drops its note that an exhausted budget fails the device in the same commit. `make test-kernel` gains a boot, limited by `vibeos.ktest=` to one test, whose `vda` has a QEMU `blkdebug` read error on one sector: that sector's read fails and a read of any other sector succeeds (F046)
...
- [ ] the portable block cache's readahead keeps a dirty page whose eviction write fails: `cache::cached_read`, which tmpfs's `tmp_rw_cache` runs, restores the evicted slot with `restore_evict` and skips the readahead when that write fails, as its demand path and `cache_init::bump_readahead` already do. Today `let _ = b.write(fill.evict_key.offset, &evict)` drops the error and refills the slot, losing the page's data. A host test with a `Backend` that fails that one write reads the evicted page back unchanged and finds it still dirty
...
- [ ] CI runs the aarch64 build and tier jobs on every push beside the x86_64 ones (§10.1 budget), on the hosted arm64 runner under TCG (§11.7), since GitHub's arm64 runners have no `/dev/kvm`
- [ ] `docs/ARCH.md` maps each row of DESIGN §11.1's seam table to its modules in each port, both halves, and lists every other architecture-specific module; `scripts/check_arch.py` in `make check` fails when a table row has no `docs/ARCH.md` row, when a file under either port's `arch/` directory in either crate is missing from it, or when a module it names does not exist, or when a skip row names neither an aarch64 counterpart nor a reason
...
- [ ] Limine 12.9 or later, pinned by commit in `setup.sh` and the CI cache keys (12.6 deletes the device-tree `memory@` nodes that contradicted the memory map; 12.9 enters at EL1, not EL2, on a CPU whose EL2 lacks VHE, so the kernel runs at EL2 only as a VHE host). aarch64 requests base revision 6, the only aarch64 revision Limine 11 and later accept; x86_64 may stay at revision 3, with `BootInfo` normalizing the difference. The full x86 ladder stays green across the bump
...
- [ ] the kernel builds for `aarch64-unknown-none-softfloat`, the counterpart of the soft-float x86 target, so kernel code never touches FP or SIMD registers and a user thread's FP state needs saving only when the CPU switches away from it (§11.6), and `scripts/check_kernel_fp.py` (§10.6) checks the aarch64 kernel ELF too, failing on any instruction that names a SIMD and FP register, FPCR, or FPSR outside the FP save and load routines, since with `CPACR_EL1.FPEN` fixed on (§11.6) nothing traps a kernel FP use; user code builds for `aarch64-unknown-none`; `rust-toolchain.toml` and the CI `targets:` inputs gain both
- [ ] the aarch64 kernel's ISA floor is FEAT_LSE and FEAT_PAN, both mandatory from Armv8.1, and `.cargo/config.toml` builds `aarch64-unknown-none-softfloat` with `-C target-feature=+lse`, for that target only, so every kernel atomic read-modify-write is one LSE instruction rather than an `ldaxr`/`stlxr` loop. The boot CPU reads `ID_AA64ISAR0_EL1.Atomic` (2 or more) and `ID_AA64MMFR1_EL1.PAN` (1 or more) right after early serial on the PL011 comes up; the code before the check (the entry, the Limine handshake, and the device-tree walk to the console) runs no atomic read-modify-write and no PAN access, and a CPU without either prints a registered refusal line naming the missing feature and halts. A secondary core checks both in its entry stub; one without them reports the missing feature through its parameter block, where its arrival flag goes, and calls PSCI `CPU_OFF`, and the boot CPU logs it and takes the bring-up failure path of §11.4. Every later Arm feature stays conditional on its ID-register field: FEAT_HAFDBS (§12.2), 16-bit ASIDs (§11.2), RAS, RNDR, BTI, pointer authentication, MTE, SVE, and ECV. User programs keep the Armv8.0 baseline Linux distributions build for, and `AT_HWCAP` reports what every CPU has (§13.10). DESIGN §3.1 records both architectures' floors. An e2e test boots `-cpu cortex-a72` (Armv8.0) under TCG and expects the refusal line and no early-exception line, which also shows that nothing before the check ran an LSE instruction. A line that brings an Armv8.0 machine into scope, such as the Raspberry Pi 4's Cortex-A72, first adds runtime selection of atomics and a PAN conditional on its ID-register field, as one box with its own primitive (AGENTS.md rule 10), and changes this test
...
### 11.2 Memory
- [ ] the page table format behind the §10.3 trait: descriptor bits, access permissions, `UXN` and `PXN`, the access flag, shareability; every kernel-half descriptor sets UXN, and a host test checks that no kernel mapping the portable page-table code builds clears it (DESIGN §5.10 rule 10 relies on it)
...
- [ ] ASIDs, so a context switch does not flush the TLB, from DESIGN §11.2's allocator, Linux arm64's generation scheme: each address space holds a generation above its ASID bits; a switch publishes the CPU's `active_asid` by a compare-exchange that succeeds only while the space's generation is current, and otherwise takes the allocator lock; a rollover, under that lock, starts a new generation, exchanges each CPU's `active_asid` for 0 and reserves what it held, so a process running at rollover keeps its number, and marks every CPU flush-pending; a flush-pending CPU runs `tlbi vmalle1`, `dsb nsh`, and `isb` before it loads any ASID of the new generation, and no rollover broadcasts a flush. `TCR_EL1.A1` is clear; ASIDs are 16 bits where `ID_AA64MMFR0_EL1.ASIDBits` reports them, with `TCR_EL1.AS` set to match; ASID 0 belongs to the empty user root and §11.4's identity map; and where 2^bits − 1 does not exceed the possible-CPU count the port uses no ASIDs, flushes ASID 0's entries on every TTBR0 switch, and logs why at boot. The allocator is portable code in `vibeos-core`. Host tests with 2-bit ASIDs and 2 CPUs force thousands of rollovers while each simulated CPU records the (ASID, address space) pairs it has run since its last flush, and fail when one ASID names two address spaces there; a loom model (§10.8) runs the switch fast path against a concurrent rollover, with a mutant whose fast path stores instead of compare-exchanging and one whose rollover skips the reservation, each of which must fail. In-guest at `-smp 4`, in the aarch64 TCG tier and under HVF as a §10.9 dev-host record, a `kernel_tests` knob caps ASIDs at 4 bits while 1,000 short-lived processes each write and check a private memory pattern; TCG keeps a CPU's soft TLB across a TTBR0 write that leaves the ASID unchanged, so a reused ASID shows there too. DESIGN §2.7's ASID row cites this box
- [ ] cache maintenance for DMA (the device-ordering box below), and for code: whenever a user page becomes executable, `dc cvau` over it (omitted when `CTR_EL0.IDC` is set), `dsb ish`, `ic ivau` (omitted when `CTR_EL0.DIC` is set), `dsb ish`, `isb`, which x86 got for free; ELF load is the first caller, and §12.2 adds the fault path
...
- [ ] device ordering and DMA coherence as DESIGN §4.7 states them: `mmio_write` runs `dmb oshst` before its store and `mmio_read` runs `dmb oshld` after its load, as Linux's arm64 `writel` and `readl` do, and x86_64's accessors carry a compiler barrier; a `_relaxed` accessor appears only with a comment saying why. Each device records whether it is DMA-coherent, from `dma-coherent` on its device-tree node or a parent bus (§11.5); for a non-coherent device, `sync_for_device` cleans the buffer to the Point of Coherency for every direction and `sync_for_cpu` invalidates it, each followed by `dsb sy`, and a DMA region smaller than a page is aligned to and sized in `CTR_EL0.CWG` granules (2 KiB when it reads 0), which boot asserts are no larger than a page. A host test records accesses, barriers, and cache maintenance through the stub `arch`, as §10.3's F016 test does, and finds an ordered `mmio_write` after the `avail.idx` store on every notify path, an ordered `mmio_read` before the ring read that follows each interrupt-status read, and, for a device marked non-coherent, a clean of every line of a buffer before the device is told of it, an invalidate of every line before the CPU reads it, and no two in-flight requests' regions in one granule. QEMU's `virt` marks its devices coherent, so a `kernel_tests` knob marks virtio-blk non-coherent, and the in-guest block tests pass on that path under TCG and, as a §10.9 dev-host record, under HVF
- [ ] the buddy, heap, and KVA allocators unchanged; that is the point of the split

### 11.3 Interrupts and time
- [ ] exception vectors: the sixteen-entry table, synchronous versus IRQ versus FIQ versus SError, and each exception decoded from its vector slot and `ESR_EL1` into the portable `TrapKind`, whose ring-3 actions DESIGN §11.5's table lists; §10.6's F005 host test extends to every exception class `0x00` to `0x3F` and the IRQ, FIQ, and SError slots; every CPU clears `PSTATE.A` once its full vector table is live, so an SError is taken where it is raised; each synchronous vector saves `ESR_EL1` and `FAR_EL1` into the frame before it unmasks DAIF (DESIGN §5.10 rule 9), and §10.6's in-guest test that yields at the top of a user fault's body passes with a data abort, each kill line naming its own fault address; DESIGN §11.5 describes the vector table and the entry as built in the same commit
- [ ] a kernel stack overflow reports itself on aarch64 as on x86_64 (DESIGN §4.5, §11.5): one KVA stack allocator on both architectures gives every guarded stack a power-of-two size S at a 2S-aligned address with the S below it unmapped, and on aarch64 every stack the kernel runs on, idle and per-CPU overflow stacks included, is 16 KiB; every aarch64 vector entry tests the stack bit before its first store and, when it is set, stashes `x0` and SP in `TPIDR_EL0` and `TPIDRRO_EL0`, switches to this CPU's overflow stack, prints `vibeOS: panic: stack overflow` with FAR, ESR, ELR, the stashed SP, and the current thread, and halts; the full vector table goes live only after the bootstrap thread has moved to a stack in that layout (§10.6); x86_64's `#DF` handler prints the same line when CR2 lies in the interrupted stack's guard. A host test of the KVA allocator checks each stack's alignment and guard. The in-guest test `df_on_ist` becomes `kstack_overflow`, one test on both architectures: a deliberate overflow of a guarded stack is caught, with the `catch` it arms, on the known-good stack (IST 1 on x86_64, this CPU's overflow stack on aarch64), with the guard address as the fault address and the kind reported as stack overflow. On aarch64 the §10.2 in-guest registry thread runs on the one 16 KiB size
- [ ] the portable IRQ model of DESIGN §5.4, on x86_64 before the GIC boxes below: drivers name an interrupt by an `IrqId`, never by a vector, through `irq::map_wired` for a firmware specifier, `irq::alloc_msi` for MSI and MSI-X, and `irq::map_percpu` for the LAPIC LVTs (and, below, the GIC PPIs); each interrupt controller is an `IrqChip` object with translate, mask, unmask, EOI, `set_affinity`, MSI allocation, `compose_msi`, and free, held in a static `BootCell` and reached as `&'static dyn IrqChip`; x86_64's 8259, I/O APICs, and LAPIC MSI domain become chips, DESIGN §5.3's device pool moves inside the x86 chip, `irq::msi_message_addr` and every other MSI composition leave the drivers, and the PCI layer writes MSI and MSI-X entries from `compose_msi`; `set_affinity`, `set_threaded`, and `free_vector` keep their names and rules and take an `IrqId`. Every driver and the Phase 6 MSI-X and affinity tests move to the new API in the same commit and keep what they assert. Host tests run the IRQ layer against a stub chip (map, allocate, set affinity, and free) and pin the GICv3 ITS command encodings (`MAPD`, `MAPC`, `MAPTI`, `MOVI`, `SYNC`) in `vibeos-core`, from the GICv3 architecture specification
- [ ] GICv2 and GICv3: distributor, redistributors, CPU interface, priorities, each an `IrqChip` (DESIGN §5.4), with priorities by DESIGN §5.3's classes: the top class reserved for §25.5's pseudo-NMI, then IPIs and the tick, then devices; a device-tree `interrupts` specifier maps to an SPI or a PPI, and the generic timer's PPI is one `IrqId` on every CPU; DESIGN §11.5 gains the INTID map (which PPIs, SPIs, and LPIs the kernel uses, and for what; its SGIs are DESIGN §5.3's table), the counterpart of §5.3's vector map, in the same commit
- [ ] MSI through the GICv3 ITS (GICv2m on GICv2), so virtio-pci and the Phase 6 MSI-X and affinity tests, on `IrqId` since the box above, run unchanged on both architectures; INTx through the device tree's `interrupt-map` as the fallback; the ITS chip maps each device by its DeviceID, from the device tree's `msi-map` or `msi-parent` (§11.5), allocates LPIs, and moves an interrupt with `MOVI` to the target CPU's collection and `SYNC`, leaving the MSI-X entry as it was, and GICv2m moves the SPI's target; the ITS tables and the redistributors' LPI property and pending tables are allocated once at boot and never freed or moved, and the kernel never clears `GICR_CTLR.EnableLPIs` (DESIGN §5.4); in-guest under `gic-version=3`, after `set_affinity` an MSI-X interrupt arrives on the new CPU while its MSI-X table entry reads unchanged, and a virtio-mmio SPI mapped from the device tree is delivered; freeing an LPI, at `free_vector` or at removal, follows DESIGN §5.4's ITS order (`DISCARD` for each event, `MAPD` with V=0, `SYNC`, a 1 s wait) before the ITT and the LPIs are freed, and a host test pins that command sequence
- [ ] the generic timer: `CNTVCT` as the cycle counter, the chosen timer's `TVAL` for the tick, the same `next_deadline` interface. The timer and its interrupt come from the device-tree timer node for the entry level: at EL1 the EL1 virtual timer (`CNTV_*`, the `virt` interrupt); at EL2 with VHE the EL2 virtual timer (`CNTV_*` redirected to `CNTHV_*`, the fifth, `hyp-virt` interrupt) when the node has one, and otherwise the EL2 physical timer (`CNTP_*` redirected to `CNTHP_*`, the fourth, `hyp-phys` interrupt), as Linux falls back. QEMU before 9.0, the `virt-8.2` and older machine types of later QEMU, and boards such as the Raspberry Pi 5 omit `hyp-virt`. A registered marker names the timer chosen, and the §11.5 parser's host tests cover four- and five-entry timer nodes; DESIGN §6.1 gains the generic timer's rows, marked aarch64, in the same commit
- [ ] a monotonic clock on `CNTVCT` with the same seqlock publication and the same host tests
- [ ] SGIs as the IPI mechanism (SGIs 0 to 7 only, mapped in DESIGN §5.3, as Linux uses them, since Arm recommends leaving 8 to 15 to the Secure world; at most eight IPI kinds on both architectures): reschedule, call-function, the panic stop of DESIGN §2.5 step 1; no shootdown SGI, because §11.2's broadcast TLB maintenance replaces it; the §10.7 F135 `panic_test` variant runs at `-smp 5` under `virt,gic-version=3` and again under `virt,gic-version=2`, with every output assertion kept, and requires `cpu 2 stopped (poll)`, `cpu 3 stopped (poll)`, and `cpu 4 not stopped`, since aarch64 has no NMI before §25.5; the owner's self-NMI step runs on x86_64 only; if CPU 3 is not `stopped (poll)` under `virt,gic-version=2`, or a numbered line follows the first `vibeOS: panic:` line, polling does not stop a serviced spinner or a CPU other than the owner writes, and DESIGN §2.5 step 1 is revised in the commit that finds it; if a poll-stopped CPU's saved frame does not reach the spin's caller, the stop routine saves the frame pointer on entry; each SGI send runs `dsb ishst` before its `ICC_SGI1R_EL1` write and `isb` after it, so the stores it publishes reach the target first (DESIGN §7.6; §11.7 cites the Arm ARM rule)
- [ ] the aarch64 idle loop checks for work with IRQs masked, runs `wfi` with them still masked, and unmasks after it wakes (DESIGN §11.1's idle row), since `wfi` wakes on a pending interrupt whatever PSTATE.I says and aarch64 has no counterpart of `sti`'s one-instruction shadow; in-guest, with this CPU's timer interrupt disabled for the test, a `kernel_tests` hook in the idle loop, between the work check and the `wfi`, queues a wake for a thread on this CPU and sends this CPU the reschedule SGI, and the woken thread runs, which the test's deadline fails when the loop unmasks before the `wfi`
...
- [ ] the per-CPU base, chosen once at boot: `TPIDR_EL1` at EL1, `TPIDR_EL2` at EL2, so that Phase 21 guests own `TPIDR_EL1`; per-CPU fields are reached through it only with IRQs masked, and `current` is not a per-CPU read: it lives in `SP_EL0` (§11.6; DESIGN §2.9 rule 5)
...
- [ ] the aarch64 context switch: `switch_context` saves and loads x19-x29, SP, and LR in `Tcb.context`, with the interrupt state from `irq_nest` as on x86_64 (DESIGN §7.5); the host test `switch_context_roundtrip` (§10.2) runs this port's switch assembly as well, from `tests/hostlib` on an aarch64 Linux host with IRQ masking supplied by a macro, so the aarch64 build job on the arm64 runner (§10.1) runs it on every push; it is built only for Linux hosts, whose assembler and object format the kernel build uses, so the macOS job compiles it out

### 11.5 Devices
- [ ] the device tree from Limine's DTB response, parsed in the portable half and host-tested like the ACPI parser: CPUs, GIC and ITS (with each PCIe host's `msi-map` and each device's `msi-parent`, which give the ITS its DeviceIDs, DESIGN §5.4), timer, PL011, PL031, PCIe ECAM with its `interrupt-map`, virtio-mmio, `/psci`, and QEMU's `fw-cfg` node, so the §10.2 command line and the `vmcoreinfo` note use the same fw_cfg interface as on x86_64; each device's `dma-coherent`, on its node or a parent bus as Linux reads it, sets its DMA coherence (DESIGN §4.7). RAM comes from the Limine memory map, never from `memory@` nodes, which Limine removes. `/chosen` is ignored as the protocol requires, so the console is the PL011 that `/aliases` names `serial0`, or, where the tree has no `/aliases` (QEMU before 9.1), the first node compatible with `arm,pl011` whose status is okay or absent, in tree order; host tests parse checked-in trees dumped with `-machine dumpdtb=` from QEMU 8.2, with and without `secure=on` (whose disabled secure UART comes first), and from a current QEMU, and each picks the UART at `0x9000000`; the parser fills the `MachineDesc` of the box below, and reads `/reserved-memory` and the FDT header's memory reservation block, whose ranges never enter the buddy, since Limine's memory map need not mark them; host tests cover a tree with both; DESIGN §7.1 gains the device tree's counterpart of its table list in the same commit
- [ ] one portable machine description, `MachineDesc` in `vibeos-core` (DESIGN §11.1): the CPUs with their hardware ids and enable method, the interrupt controllers, the timers, the consoles, the PCI host bridges, and the reserved memory, filled by the device-tree parser above and, on x86_64, by the §2.4 ACPI parser from the MADT, HPET, and MCFG; SMP bring-up, the IRQ layer, and the §6.1 device registry read only it, and x86_64's consumers of the ACPI parser's results move onto it in the same PR. Devices bind through the registry to a firmware-node handle, a device-tree node here and an ACPI namespace node from §20.2, matched by compatible string or `_HID`, so their resources and INTx routing are read from that node, not from `MachineDesc`. Host tests fill it from the checked-in QEMU `virt` trees above and from the x86_64 ACPI tables §2.4's host tests read, and the full x86 ladder stays green
...
### 11.6 User mode
- [ ] `svc` entry and `eret` exit, every entry from EL0 (`svc` and each synchronous exception, IRQ, and SError taken at EL0) saving the aarch64 user frame of DESIGN §5.10 at the top of the thread's kernel stack, `orig_x0` and the syscall number included, and a restart rewinding it (`x0` from `orig_x0`, `pc` back 4); the return to EL0 sets all of DAIF (D, A, I, and F) before it writes `ELR_EL1`, `SPSR_EL1`, or `SP_EL0` and keeps it set until `eret`, with a debug-build check before the `eret`, as §10.6 requires IF=0 of the x86_64 syscall exit (DESIGN §11.5); the `svc` handler unmasks IRQs once the frame is saved, so the syscall body runs with IRQs on, as DESIGN §2.9 rule 3 requires on both architectures; DESIGN §5.10's aarch64 table and §11.5 describe the entry and exit as built in the same commit; in-guest, a `kernel_tests` hook sets `MDSCR_EL1.MDE` and `KDE` and arms a hardware breakpoint on the exit's `eret`, and a user loop of 10,000 syscalls finishes with the breakpoint never taken, which fails when the exit masks only `DAIF.I`
- [ ] `current` in `SP_EL0` (DESIGN §2.9 rule 5), where Linux arm64 keeps it: every entry from EL0 saves the user `SP_EL0` into the user frame, then loads `PerCpu.current` through `TPIDR_ELx` into `SP_EL0` while DAIF is still set; an entry from EL1 (EL2 under VHE) leaves `SP_EL0` alone; the switch writes the incoming TCB pointer into `SP_EL0` beside `switch_context`; the return to EL0 restores the user `SP_EL0` from the frame after it sets DAIF, since `SP_EL0` is the user stack pointer. `arch::current_tcb()` is `mrs x, sp_el0`; `arch::cpu_id_hint()` loads `PerCpu.cpu_id` through `TPIDR_ELx`. DESIGN §7.5's aarch64 per-thread rows say so in the same commit. §10.3's `current_at_if1` and migration tests pass in `make test-kernel-smp4 ARCH=aarch64`, and a user thread that checks its stack pointer against a copy in memory finds it intact across 10,000 syscalls and timer preemptions
...
- [ ] PAN enabled on every CPU, with no condition since FEAT_PAN is in the §11.1 ISA floor (unlike SMAP's CPUID check on x86_64), and with `SCTLR_EL1.SPAN` clear so every exception entry to EL1 sets PAN again, and cleared only inside the §10.6 accessors (SMAP's equivalent); PXN on every user page (SMEP's); the §10.6 user-memory tests pass unchanged, except the x86-only `popf` variant, which has no counterpart because EL0 cannot write PAN; that variant and §10.6's `swapgs` `#DB` and `iretq` `#GP` tests are on the `docs/ARCH.md` skip list; the `swapgs` `#DB` test's row names the `svc` box's exit-window breakpoint test as its aarch64 counterpart, and the `iretq` `#GP` test's row names the ELR test of the ring-3 box at the end of this section
- [ ] `TPIDR_EL0` for user TLS; the ELF loader's TLS layout handles variant I on aarch64 and variant II on x86_64
- [ ] the user TLS base is per-thread state on both architectures, in the commit that adds `TPIDR_EL0` (AGENTS.md rule 8): `on_switch` saves `FS_BASE` (`TPIDR_EL0` on aarch64) for an outgoing user thread and loads it for an incoming one, `fork` and this section's `clone` copy the caller's saved value rather than the live register, `execve` sets the new image's value, and `force_kernel()`'s selector load no longer leaves the next user thread at base 0; DESIGN §7.5's two rows describe it as built in the same commit. In-guest on each architecture, pinned to one CPU as §10.6's `enter_user_full` test pins its children to CPU 0: a `PT_TLS` process and a process with no `PT_TLS` segment, whose thread pointer is 0, alternate with `sched_yield`, and each reads back its own thread pointer after every switch; on aarch64, two processes that each write their own value to `TPIDR_EL0` with `msr` alternate the same way and each reads back its own (F022)
- [ ] the aarch64 syscall convention documented in `docs/SYSCALL.md`: `svc #0`, number in the low 32 bits of `x8`, read as unsigned as Linux's arm64 `invoke_syscall` reads it, arguments in `x0` to `x5` converted to their C types as on x86_64 (SYSCALL.md §1), result in `x0`, errnos as on x86_64. The numbers are the asm-generic table Linux uses on arm64, which musl's aarch64 port and static Linux binaries need. Numbers are necessary but not sufficient: the per-architecture argument order (§10.5) and struct layouts (below, §13.6, §13.8) do the rest
...
- [ ] user FP and SIMD state (V0-V31, FPCR, FPSR) follows DESIGN §7.5's FP binding, which §10.6 built and host-tested on x86_64; this port adds only the save and load of those registers. Every CPU sets `CPACR_EL1.FPEN` to `0b11` (`CPTR_EL2`'s field under VHE) at bring-up, before any user thread exists, and never changes it (Limine enters with `CPACR_EL1` zero), so no FP or SIMD access traps, as on Linux arm64; the state is saved at every switch away from a thread whose state is live, and loaded before the thread's next return to EL0 when the registers hold another thread's state. Every aarch64 user compiler emits NEON, so a first-use trap would set up nothing that creation cannot, and trap-driven lazy switching would save nothing and is the scheme LazyFP (CVE-2018-3665) retired on x86; the kernel itself never touches those registers (§11.1). In-guest, on one CPU: process P fills V0-V31 with a pattern and yields in a loop while a new process Q starts, and no V register Q reads at its first instruction holds P's pattern; §10.6's migration test runs on aarch64 with its counter in `v0`
...
- [ ] ring 3 cannot halt the kernel on aarch64 (DESIGN §11.5): in-guest, children that execute `brk #0`, `udf #0`, and a branch to an odd address end with `SIGTRAP`, `SIGILL`, and `SIGBUS` in their `wait4` status, and a `kernel_tests` hook that sets a syscall's saved ELR to 2^48 finds the process killed by `SIGSEGV` and the kernel running
...
- [ ] the panic path, backtrace, and symbol table working on aarch64: a frame-pointer walk along the `x29` chain that follows `x29` only into aarch64's known stacks, listed beside §10.7's x86_64 list: the current thread's KVA stack, the recorded boot stack, and this CPU's §11.3 overflow stack; host tests cover both lists
...
- [ ] every job that boots an aarch64 guest (the tier jobs, and the aarch64 legs of the nightly and weekly jobs) runs on an arm64 runner, `ubuntu-26.04-arm` (§10.1) or the scheduled macOS job's arm64 image, never on an x86_64 runner: TCG adds no ordering to an aarch64 guest's loads and stores, so on an arm64 host the host's weak ordering can reach guest code, while an x86_64 host runs them in its TSO order and hides every reordering but store-to-load. `scripts/check_workflows.py` fails a job that runs a `test-*` or `run` target for aarch64 (an `ARCH=aarch64` argument or a matrix `arch` of `aarch64`) unless its `runs-on` names an arm64 image, and `tests/harness/test_workflows.py` covers the rule; DESIGN §8.6 records which runner runs each architecture's jobs. The jobs run under TCG: GitHub-hosted arm64 runners have no `/dev/kvm` (the request was closed as not planned); native-speed aarch64 runs are HVF on the dev host, kept as §10.9 dev-host records
- [ ] the timing tests pass under HVF on the dev host, the same fix as the §10.1 KVM leg
- [ ] the weekly smp-stress job on both architectures, its aarch64 leg on `ubuntu-26.04-arm` (above)
- [ ] an in-guest test `weak_order_probe` (`kernel_tests`) runs two litmus shapes on two CPUs with `Relaxed` accesses and no barrier: message passing (one CPU stores data, then a flag; the other loads the flag, then the data) and load buffering (each CPU loads one location, then stores the other). It runs 10^7 iterations of each shape or 5 minutes, whichever ends first, prints each shape's iteration count and weak-outcome count, and fails only when a shape completes no iteration. `make test-smp-stress ARCH=aarch64` runs it in one boot of its own (`VIBEOS_KTEST=weak_order_probe`), so the weekly job's aarch64 leg and the HVF record below run it, and each writes the counts to its job summary or record and to §10.9's `ci-history`; no per-push tier selects it. If the arm64 runner's counts for both shapes are zero in each of its first four weekly runs, the commit that reads them changes the runner box above to say that the HVF records are the only weakly ordered execution in any gate
- [ ] from Phase 11 on, `make gate PHASE=N` also checks the entries of `tests/gates/common.toml` at the gated commit, beside phase N's map: two dev-host records, `VIBEOS_QEMU_ACCEL=hvf make test-kernel-smp4 ARCH=aarch64` and `VIBEOS_QEMU_ACCEL=hvf make test-smp-stress ARCH=aarch64`, each run by `make gate PHASE=N RECORD=1` in a loop for 30 minutes, with the pass count in the record; any failed run fails the record. Every later phase lands code on both architectures, and these records are the only gate that runs the -smp 4 tier, whose publish-last tests (§10.10) and `current` tests (§10.3) matter most on a weakly ordered CPU, on the CPU itself. `common.toml` entries are keyed by name, not by gate-line text, and `scripts/check_gates.py` checks that each names a `make` target that exists
- [ ] a litmus test under `tests/litmus/` for each barrier idiom the aarch64 port uses: the §2.7 seqlock publish and read, the log ring publish, `SpinMutex` release and acquire, the §10.4 wake inbox hand-off, the §11.2 ASID switch's compare-exchange of `active_asid` against a rollover's exchange, and the virtio descriptor write before the avail index update. herd7 with Arm's `aarch64.cat` shows the bad outcome forbidden, a twin with the barrier removed shows it allowed, and the x86_64 versions run under `x86tso.cat` to record which barriers x86 elides
- [ ] the litmus set also covers the idioms the kernel review found unsound on a weakly ordered CPU: the seqlock writer's release fence after the odd bump, written from the instructions the kernel build emits (an LSE `ldadd` for `fetch_add`, since the kernel builds with `+lse` (§11.1), with the `ldaxr`/`stlxr` loop the target emits without it kept as a second encoding of the same test); call-function slot reuse across two rounds; and the store-then-load pairs that need the full `dma_mb` (the avail index store before the `avail_event` or `used.flags` load, and the `used_event` store before the `used.idx` reload) (F016, F098, F109)
- [ ] each barrier helper in the aarch64 port names its litmus test in a doc comment, and `scripts/check_litmus.py` in `make check` fails when a helper names none or names a missing test; an idiom herd7 cannot express (TLB maintenance, the `dsb` before an SGI system-register write, the `dmb oshst` before an `mmio_write` and the `dmb oshld` after an `mmio_read`, which order Normal memory against the Device memory herd7 does not model, and the cache maintenance of non-coherent DMA) cites the Arm ARM rule instead. `make litmus` runs herd7, pinned and installed from opam on Linux and macOS, on the nightly job beside `make models` (§10.8)
...
- [ ] a third port that costs a week says the seam is right; one that costs a month says where it is wrong; each change the port needs outside the two `arch/` directories becomes a missing or corrected row of DESIGN §11.1's seam table, in the same commit
...
- [ ] the same fault tests pass on x86_64 and aarch64, with the error code and `ESR_EL1` decoded into one `TrapKind`, from which the shared fault path works out one fault kind
...
- [ ] per-process and global fault counters by fault kind, both reported through syscall 500 until §13.9's procfs, where each process's are `/proc/<pid>/vibeos/faults` and the global ones `/proc/vibeos/faults`; `/proc/<pid>/stat` and `getrusage` carry only Linux's `minflt` and `majflt`, the faults that needed no I/O and those that did
...
- [ ] every CPU in that set has invalidated and acked a user-mapping change before anything relies on it (DESIGN §2.4): an unmap, a permission reduction, the COW write-protect, a `MAP_FIXED` replacement, a dirty-bit clear, and a §12.6 reverse-map unmap each put the frames and page-table pages they drop in the operation's gather (DESIGN §4.3), invalidate locally, send one ranged round to the CPUs in that set (DESIGN §7.9; broadcast `tlbi ...is` on aarch64, §11.2), and wait for the acks. Only then does the gather release its units, do `mprotect`, `munmap`, `mremap`, and `madvise(MADV_DONTNEED)` return, does `fork` make the child runnable, and does writeback count a page as clean; `unmap_free` and `teardown_pool` stop freeing frames inside the page walk; §21.2 extends this path to a VM's second-level entries (DESIGN §2.4)
...
- [ ] one page cache of mappings (DESIGN §10.6), rather than a block cache and a page cache disagreeing: every cached page belongs to one file mapping (file data, tmpfs, the ELF pages §12.2 maps) or one block-device mapping (filesystem metadata and the device node) at one page index, and a device location is never a key; today's `(dev_id, page offset)` cache becomes each block device's mapping, and one frame pool, one LRU, and one set of writeback threads serve both kinds; a regular-file `read` or `write` moves min(count, `MAX_RW_COUNT`) bytes in one call and stops early only at end of file, at a fault in the user buffer, or on a fatal signal, as Linux does, instead of today's 256 bytes per call (SYSCALL.md §3.1); an in-guest test reads a 1 MiB file with one `read` and gets 1,048,576
...
- [ ] page-cache pages, today's block-cache slots first, carry a FILLING state (key visible, readers wait on the slot's §10.11 wait queue, which replaces the yield spin) beside §10.11's WRITEBACK state; a host test interleaves `plan`, `install`, and writeback from two callers and finds no duplicate key and no stale read (F015)
...
- [ ] one bottom-half thread per threaded vector, pinned to the CPU the vector is routed to and moved by §6.3's `set_affinity`, replacing the one `irqth` thread on the last online CPU (DESIGN §5.4); a bottom half takes no sleeping-tier lock, waits for no I/O completion, its own device's included, and allocates without direct reclaim, in DESIGN §4.4's atomic class; each of its waits for its own device's resources, such as a free descriptor, has a deadline and ends when its vector's quiesce flag is set (DESIGN §5.4, §10.3), after which it returns without touching the device; a block completion allocates nothing, since what it needs was allocated at submission (the owned-submission box above, F042). In-guest at `-smp 4`: virtio-blk with 4 queues completes I/O submitted from every CPU, each queue's bottom half running on its vector's CPU by per-thread CPU counters; and one virtio-blk queue's bottom half held at a `kernel_tests` hook delays neither the other queues' bottom halves nor virtio-rng's; in a debug build, `IoWaiter::wait` and the page and slot waits assert, beside §10.3's might-sleep checks, that the caller is neither a bottom half nor a softirq-equivalent item, and an in-guest test calls `IoWaiter::wait` from a bottom half and from a softirq-equivalent item under `arch::catch::catch_panic` and finds the assertion each time
- [ ] block error handling (DESIGN §10.3): each request carries a deadline, 30 s by default and settable per device, that starts when the driver dispatches it. One error-handler thread per virtio-blk function, a no-reclaim thread, wakes at the earliest deadline, at `DEVICE_NEEDS_RESET` (which §10.11's `finish` answers until this box by failing every request), and at a used-ring check failure, and recovers in DESIGN §10.3's order: `Resetting`; each queue quiesced (its interrupt masked, its quiesce flag set, its bottom half idle); the reset (status 0, read back as 0, polled with sleeps for at most virtio-blk's 1 s reset bound); every dispatched request claimed; then the rings reinitialized and the requests resubmitted in each queue's submission order, or, when the reset fails, bus mastering cleared and read back or the buffers quarantined, the requests completed with `Io`, and the device `Failed`. Exactly one party claims each request: the claim's state machine is in the portable half, and a loom model (§10.8) runs it with three claimants (harvest, error handler, removal), with a variant that loads the state and then stores it, which the model must fail. Each resubmission uses the request's retry budget, and a request whose deadline passes twice completes with `Io`. `IoWaiter::wait` parks with the device's stall bound S as its deadline and logs a report naming the request if it passes. In-guest at `-smp 4` with 4 queues and the deadline set to 1 s: (a) a `kernel_tests` hook discards one used-ring entry; the request is resubmitted after the reset and succeeds, the next request on the same queue succeeds, and the hook records that nothing touched the request's buffer between its deadline and the status reading back 0; (b) a hook posts a request's used entry just as its deadline passes; the request completes once, the handler finds it claimed after the quiesce and resets nothing, and the `IoWaiter` and free-frame counts return to baseline; (c) queue 0's bottom half is parked at a hook on a descriptor wait when queue 2 times out; the reset starts only after queue 0's bottom half has woken to its quiesce flag and gone idle, and both queues serve I/O afterwards; (d) a request whose used entry the hook discards on every dispatch completes with `Io` after its second timeout, its buffer is released only after the status read back 0, and the device stays `Bound`; (e) a hook keeps the status from reading 0: within the device's S the device is `Failed`, `fsync` on the mounted vibefs volume returns `EIO` and the volume commits no more, and every thread the test blocked on the device has returned. The driver emits the registered `vibeOS: block: <dev> timeout` and `vibeOS: block: <dev> reset` lines, which fail any run whose test does not declare them (DESIGN §8.3), and each case records how long the reset took under TCG on the hosted runners, which checks virtio-blk's 1 s reset bound: a reset that takes more than half of it raises R in that commit, and S follows. A run that the failure-line rule fails under runner load is recorded as a flaky-test line (DESIGN §9.8); the deadline changes only when a measured reset time justifies it, never to make CI green
- [ ] block completion runs in two stages (DESIGN §10.1): the claiming party wakes waiters and settles page state, and verification, decryption, stacked completions, and retries run as softirq-equivalent items that are part of their request. The high-priority workqueue takes such an item without allocating, so queuing one cannot fail; a host test of the portable workqueue queues 1,000 embedded items while its ring is full and runs each exactly once. In-guest, 256 concurrent reads whose fills a `kernel_tests` hook routes through a stage-2 item all complete, and no page leaves FILLING before its item has run
...
- [ ] writeback errors reach `fsync` as DESIGN §10.6 says, with Linux's `errseq` semantics: an error in writeback, or in the commit that would make a file's data durable, is recorded on the file's mapping and on its volume, each open file description's next `fsync` or `msync` with `MS_SYNC` returns it once, and a description opened later returns an error no description has yet seen; vibefs keeps the failed data dirty and writes it to other blocks at its next commit, withholding until unmount the blocks whose writes failed, and a volume whose commits fail `DEFAULT_RETRY_BUDGET` times in a row, or whose device goes `Failed`, goes read-only and drops its dirty pages with the error recorded (VIBEFS.md §10). In-guest, through `Vfs`'s per-inode `fsync`: a `kernel_tests` hook fails every write to `vda` while a background commit writes a dirty vibefs file, and is cleared before the next commit; after that commit succeeds, `fsync` on each of two descriptions opened before the failure returns `EIO` once and then 0, a description opened after both have seen the error returns 0, and after a remount the file holds its data. With the hook left on, the volume goes read-only after `DEFAULT_RETRY_BUDGET` failed commits in a row, and each description's next `fsync` returns `EIO`
- [ ] stable pages, as DESIGN §10.6 says, for every file whose data is checksummed (vibefs v1's extent CRCs, v2's block checksums): writeback write-protects the page's user mappings through §12.1's reverse map and completes the §12.3 shootdown before it checksums the page, and a `write()`, a write fault, or a kernel write into a page under WRITEBACK waits for that writeback to complete. In-guest, one thread rewrites a `MAP_SHARED` page of a vibefs file in a loop while another calls `msync` with `MS_SYNC` 1,000 times, and after a `kernel_tests` hook drops the page cache every read of the file succeeds, with no `Corrupt`; §19.8 runs this test again once its zero-copy path has replaced the bounce copy. A `Corrupt` read in this test or in §19.8's `O_DIRECT` test, or data lost after the writeback-error box's hook clears, reopens DESIGN §10.6's writeback contract
- [ ] one LRU with second-chance aging, which §12.6 reclaims from; §19.10 splits it into an active and an inactive list
- [ ] readahead driven by detected access patterns; a readahead fill that needs verification or decryption completes in stage 2 (DESIGN §10.1)
...
- [ ] a crash-state enumerator on the host rebuilds the disk from a trace of §10.2's volatile-cache device: after each flush epoch, the durable prefix plus every subset of that epoch's writes up to eight, a seeded sample of larger subsets, and every superblock write torn at a 512-byte sector boundary, so each state costs a host mount, not a boot; the host tests of the portable volume run the same enumeration beside `CrashDisk`'s suffix drop
...
- [ ] one reserve, R frames of the buddy's free count, sized at boot as Linux sizes `min_free_kbytes` (DESIGN §4.4): a general allocation stops at R, an atomic-class one (IF=0, a spinlock held, or an RCU read-side section; a softirq-equivalent item; a threaded bottom half) at R/2, and a progress-class one (the §12.5 writeback threads, §12.7's swap-out thread, a thread while it runs direct reclaim, §19.10's background reclaim thread) may use all of it; `meminfo` shows R and each class's low-water mark. Each dedicated progress-class thread is listed here as it lands, with the most it allocates for one unit of its work, zero where it allocates nothing: the §12.5 writeback threads, the OOM reaper, and later §12.7's swap-out thread and §19.10's background reclaim thread. R is at least twice the sum of those bounds and is recomputed when such a thread starts or stops (DESIGN §4.4). In `kernel_tests` builds each unit records its peak draw, and a unit past its listed bound fails the test that ran it, since the bound is then wrong; §19.10's low-water marks show whether twice the sum is enough. A fault or `mmap` allocates the page-table pages it may need before it takes the page-table spinlock, with reclaim allowed, and frees those it did not use. In-guest: a `kernel_tests` softirq-equivalent item allocates frames and holds them until an allocation fails, while a kernel thread writes a file on `vda` through `Vfs` past the dirty limit and calls its per-inode `fsync` (§12.5); the atomic class's low-water mark is at least R/2, the `fsync` returns, and the progress class's low-water mark is above zero
...
- [ ] allocation failure follows DESIGN §4.4: a fallible allocation that still fails after direct reclaim, the OOM killer, and the wait for its victim returns `ENOMEM` to its caller, which unwinds what it built, and a kernel thread with no caller handles it as DESIGN §4.4 says; only a boot-time allocation, made before `irq: enabled`, panics, and it prints the full memory state from §12.1's accounting; after `irq: enabled` no allocation failure panics
- [ ] a pending `SIGKILL` ends a process that only touches memory: §10.6's check on every return to ring 3 also runs on every return to EL0, with IF=0 as DESIGN §5.10 rule 11 requires, and §10.6's exit-work hook test passes on aarch64; the page-fault handler checks for it before it allocates; and a kill aimed at a thread running on another CPU sends that CPU the §4.9 reschedule IPI (an SGI on aarch64); §13.8 extends this path to handled signals
...
- [ ] in-guest, with §10.4's allocation-failure hook armed at zero from the moment the OOM killer chooses its victim: a victim that holds open files on a FAT volume on `vda`, one of them unlinked, and both ends of a pipe is killed and exits, its parent's `wait4` reports `SIGKILL`, the hook sees no allocation by the victim's threads after exit's first release, and the kernel stays up; once the hook is disarmed, the deferred release of the unlinked file completes, and after shutdown the host's `fsck.fat -n` finds no lost cluster on the image (DESIGN §4.4). An allocation by the victim after exit's first release, or a deferred release that never completes once the hook is disarmed, shows an exit path the rule misses; that path then gets a creation-time allocation or a listed progress bound, recorded in DESIGN §4.4
...
- [ ] TLS through `clone`'s `tls` argument (the fifth argument on x86_64, the fourth on aarch64): `FS_BASE` on x86_64 and `TPIDR_EL0` on aarch64, plus `arch_prctl(ARCH_SET_FS)` for musl on x86_64; `arch_prctl(ARCH_SET_FS)` and `clone` refuse a base at or above `USER_MAP_END` with `EPERM`, as Linux refuses one at or above `TASK_SIZE_MAX`, so `wrmsr IA32_FS_BASE` never takes a non-canonical value; a `clone` without `CLONE_SETTLS` gives the child the caller's saved base, as §11.6's `fork` does
...
- [ ] `FUTEX_WAIT`, `FUTEX_WAKE`, `FUTEX_WAIT_BITSET`, and `FUTEX_WAKE_BITSET` on a user address, with a hash table of wait queues; the key is computed, with its region lookup under the address-space lock, and that lock released before the bucket lock is taken; the word is read under the bucket lock through §12.5's non-faulting accessor, and on a fault the waiter drops the bucket lock, faults the page in with no lock held, and retries, as Linux's `futex_wait_setup` does (DESIGN §2.9 rule 4)
- [ ] each operation and flag as futex(2) defines it at the baseline: `FUTEX_WAIT`'s timeout is relative and `FUTEX_WAIT_BITSET`'s an absolute deadline, each on `CLOCK_MONOTONIC` unless `FUTEX_CLOCK_REALTIME` is set; `FUTEX_WAKE_BITSET` wakes only waiters whose bitset shares a bit with its own, `FUTEX_WAIT` and `FUTEX_WAKE` act as the all-ones bitset, and a zero bitset returns `EINVAL`. `FUTEX_PRIVATE_FLAG` is accepted on every operation and selects the private key of the next box. `FUTEX_CLOCK_REALTIME` is accepted on the operations the baseline accepts it with, `FUTEX_WAIT_BITSET` among them, and follows §13.9's clock-step rule; with any other operation it returns `ENOSYS`. An unknown operation returns `ENOSYS`, and so do the six priority-inheritance operations until §19.4 lands them, as on Linux built without `CONFIG_FUTEX_PI`, so glibc and LTP read them as absent. Rust's `std` waits with `FUTEX_WAIT_BITSET | FUTEX_PRIVATE_FLAG` and an absolute `CLOCK_MONOTONIC` deadline and reads any error other than `ETIMEDOUT` or `EINTR` as a wake, so without these its contended `Mutex` and idle `Condvar` spin; glibc's timed waits use them too
...
- [ ] in-guest: a forked parent and child wait and wake on the same private address without seeing each other's wakes; two processes mapping one `shm_open` object do see them; a waiter survives a COW break on its page; the gate's cross-process mutex test; a `FUTEX_WAKE_BITSET` wakes only the waiters whose bitset shares a bit with its own and returns their count, a zero bitset returns `EINVAL`, a `FUTEX_WAIT_BITSET` with an absolute `CLOCK_MONOTONIC` deadline 100 ms ahead returns `ETIMEDOUT` no earlier than that deadline, and an unknown operation returns `ENOSYS`
...
- [ ] `rt_sigaction` honors `SA_RESTORER`. On x86_64 it is the only return path: a handler installed without it is not run, and its signal is dropped for the forced `SIGSEGV` of DESIGN §5.10 rule 12, as on Linux; nothing is ever written to the NX stack as code. On aarch64, a handler without a restorer returns through a read-only, user-executable trampoline page the kernel maps into every process at `exec`, so glibc-static binaries work; the page holds Linux's `mov x8, #139` and `svc #0`, the pair libgcc's and LLVM's unwinders match to step through a signal frame, and §13.10's vDSO exports it as `__kernel_rt_sigreturn`. The user crate gains `sigaction`, `sigprocmask`, and a restorer stub per architecture, which the gate's handler test uses
- [ ] `rt_sigprocmask`, `rt_sigpending`, `rt_sigsuspend`, `rt_sigtimedwait`
- [ ] handled signals take the §12.6 `SIGKILL` path: a signal is acted on at syscall exit and at interrupt and exception exit to ring 3 or EL0 (DESIGN §5.10 rule 11's exit work), a signal sent to a thread running on another CPU sends that CPU a reschedule IPI (an SGI on aarch64), and a thread in an interruptible wait is woken
- [ ] delivery builds a Linux `rt_sigframe` on the user stack (`siginfo`, `ucontext` with `uc_mcontext`, and the FP state), runs the handler, and returns through `rt_sigreturn`; the `ucontext` layout per architecture is pinned by a host test, since musl reads it; delivery copies the interrupted context from the thread's user frame (DESIGN §5.10) into the `ucontext` and rewrites the frame to enter the handler, and `rt_sigreturn` writes the saved context back into it, RCX and R11 included, so the return to the interrupted code takes `iretq` whenever RCX or R11 differs from RIP or RFLAGS; the handler starts with the registers, flags, and FP state DESIGN §5.10 rule 12 gives it
...
- [ ] signal delivery (the `sa_handler` and `sa_restorer` addresses) and `rt_sigreturn` (the whole restored frame) validate the user context with DESIGN §5.10 rule 10's validator, pure code in each port's `vibeos-core` half with a host test for each refused or cleared field on both architectures, before the return to userspace. On x86_64: RIP canonical and below `USER_MAP_END`, CS taken from the frame with RPL 3 and `SIGSEGV` unless it is then `0x33`, and SS, when the frame's `uc_flags` has `UC_STRICT_RESTORE_SS`, taken the same way with `SIGSEGV` unless it is `0x2b`, and otherwise set to `0x2b`, which is what Linux gives with no compat or LDT code segment; delivery sets `UC_SIGCONTEXT_SS` and `UC_STRICT_RESTORE_SS` in every frame, as Linux's 64-bit frames carry them, RFLAGS masked to user-settable bits (never IOPL, NT, VM, or RF from the frame), MXCSR masked to `MXCSR_MASK`, and in the XSAVE header `XSTATE_BV` a subset of the user features in XCR0, `XCOMP_BV` zero, and the reserved bytes zero, as Linux's `validate_user_xstate_header` requires, before the image reaches `fxrstor`/`xrstor`; a fault on that restore, wherever the kernel performs it, goes through an exception-table fixup that loads the initial FP state and delivers `SIGSEGV`. On aarch64: SPSR's reserved bits cleared, SS and IL never taken from the frame, and `SIGSEGV` for a frame whose SPSR is not AArch64 EL0t or sets any of D, A, I, or F, as on Linux arm64; FPCR's and FPSR's reserved bits cleared
- [ ] in-guest, per architecture: `rt_sigreturn` with a non-canonical PC, kernel selectors, EL1 mode or DAIF.I set in `pstate`, the compat code selector `0x23`, IOPL set, reserved MXCSR bits, or an XSAVE image with an `XSTATE_BV` bit outside XCR0, a nonzero `XCOMP_BV`, or a nonzero reserved header byte gets `SIGSEGV` and the kernel stays up; on x86_64 a frame whose `uc_flags` lacks `UC_STRICT_RESTORE_SS` and whose SS is 0 resumes with SS `0x2b`; a handler that interrupts a leaf function holding live data in its red zone leaves that data intact
- [ ] in-guest on both architectures, and on the §10.1 KVM leg: a loop that checks a canary in every general-purpose register (RCX and R11 included on x86_64, `x0`-`x30` on aarch64) runs through 10,000 `SIGALRM` deliveries whose handler returns through `rt_sigreturn`, and never finds a register changed
- [ ] handler entry follows DESIGN §5.10 rule 12, in-guest on both architectures and on the §10.1 KVM leg, and each program below also runs in the §13.11 differential runner, so Linux checks the rule: on x86_64 a `SIGALRM` handler that interrupts a loop running under `std` with MXCSR's rounding bits changed finds DF clear, MXCSR `0x1F80`, and FCW `0x037F`, and after `rt_sigreturn` the loop finds DF set and its own MXCSR; on aarch64 an `SA_SIGINFO` handler finds `x0` to `x2` and `x30` as the rule gives them and FPCR as the interrupted code left it; on x86_64 a child that sets TF with `popf` under a `SIGTRAP` handler gets exactly one `SIGTRAP` per instruction of a 100-instruction sequence with no syscall in it, each at the next instruction's address, and none from inside the handler; and an x86_64 user-crate program that sets TF under a `SIGTRAP` handler and runs a sequence containing `syscall` prints each `SIGTRAP`'s address, which must match Linux's (aarch64 has no counterpart, since EL0 cannot enable its own single step). A divergence from Linux in the differential runner changes DESIGN §5.10 rule 12 to Linux's behaviour

- [ ] a signal that cannot be delivered becomes DESIGN §5.10 rule 12's forced `SIGSEGV` and is never retried, and a synchronous fault signal's `siginfo` lives in a per-thread slot, never a queue entry. In-guest on both architectures: a process whose stack pointer points at an unmapped page, with a `SIGSEGV` handler on that stack, pushes, and dies by `SIGSEGV` within 1 s with the kernel up; a process with a `SIGUSR1` handler on that bad stack and a `SIGSEGV` handler on its `sigaltstack` sends itself `SIGUSR1` with a raw `tgkill`, and the `SIGSEGV` handler runs with `si_code` `SI_KERNEL`; and with a `kernel_tests` hook failing every kernel allocation, an undefined instruction under a `SIGILL` handler delivers `si_code` `ILL_ILLOPN` on x86_64 and `ILL_ILLOPC` on aarch64, as on Linux
- [ ] real-time signals with queueing, since standard signals coalesce and that surprises people; queued signals count against `RLIMIT_SIGPENDING` as getrlimit(2) documents, and a send past it that would queue fails with `EAGAIN`; a synchronous fault signal never takes a queue entry or counts against that limit (DESIGN §5.10 rule 12)

- [ ] interaction with blocking syscalls: interrupt with `EINTR` or restart, per `SA_RESTART`; a restart rewinds the user frame as DESIGN §5.10 says before delivery saves it, so `rt_sigreturn` resumes into the restarted call
- [ ] per-thread signal masks with process-directed signals delivered to an eligible thread
- [ ] the `core` default action writes an ELF core in Linux's per-architecture note layouts: one `NT_PRSTATUS` per thread, copied from that thread's user frame (DESIGN §5.10), `NT_PRFPREG`, `NT_PRPSINFO`, `NT_SIGINFO`, `NT_AUXV`, and `NT_FILE`, plus the writable and anonymous mappings, to the path `kernel.core_pattern` names, set at boot by Linux's `sysctl.kernel.core_pattern=` on the §10.2 kernel command line and from §23.4 through `/proc/sys/kernel/core_pattern` (`%p` and `%e` expanded; `core` in the working directory when absent, as on Linux), under `RLIMIT_CORE`. The writer stops every other thread of the process and waits for each one's `on_cpu` flag to clear before it reads that thread's registers (DESIGN §7.5); a core of a four-thread process whose threads loop with distinct values in `xmm0` (`v0` on aarch64) records each thread's value in its `NT_PRFPREG`. A process §13.9 marks not dumpable leaves none. The core file is created with the dumping process's fsuid and fsgid, mode 0600, and `O_NOFOLLOW`, and is never written through a symlink, a file that is not regular, a file with more than one link, or a file another uid owns, as core(5) documents; an in-guest test plants a symlink and a hard link, as uid 1000, at the core path of a root process that then dumps, and finds both targets unchanged and no core written. Host `gdb` and `lldb` open it against the binary
- [ ] utest runs attach a scratch FAT32 image as a second virtio-blk disk, which `/sbin/init` mounts at `/core` with §13.9's `mount`, and set `sysctl.kernel.core_pattern=/core/%e.%p`; after QEMU exits the harness copies each core off it and prints a host backtrace beside the failing utest (`gdb-multiarch` in CI, `lldb` on the macOS dev host)

### 13.9 POSIX floor
- [ ] a tracked list of the syscalls needed to build and run the target software set: every number the baseline defines for each architecture has a row in the §10.5 table with the same number, marked implemented, partial, or `ENOSYS` by design with the fallback its known callers take. The numbers come from a fact table checked in beside the §10.5 table, one per architecture, which a script generates from the `asm/unistd_64.h` (x86_64) and `asm/unistd.h` (arm64) that `make headers_install` writes from the baseline release's kernel.org tarball, the one §12.4's oracle build fetches, and which names the release, the tarball's SHA-256, and the header path (DESIGN §1.5); the edit that moves the baseline regenerates it. `make check` fails on a missing row, a differing number, a row whose number the baseline does not define (SYSCALL.md §8), or a name that a pinned copy of musl's `arch/<arch>/bits/syscall.h.in` gives a different number; `docs/SYSCALL.md` §3 renders the status, and the §13.11 runner lists in its job summary every `ENOSYS` the corpus hits
...
- [ ] `umount2` takes Linux's flags: `MNT_DETACH` unpublishes the mount and every mount below it at once, and each is freed at its last reference (DESIGN §2.11 rule 3), so files, working directories, and roots reached through a detached mount keep working; `UMOUNT_NOFOLLOW` does not follow a final symlink; and `MNT_FORCE` and `MNT_EXPIRE` act as Linux defines them, `MNT_FORCE` unmounting a local filesystem as a plain call does. In-guest: a process whose working directory is inside a vibefs mount detaches it with `MNT_DETACH`; its path no longer resolves to the mount, the process still reads a file it holds open there and opens a sibling by relative name, and the volume is synced and released at the last close
...
- [ ] `/dev/kmsg` in devfs, so `dmesg` reads the kernel log; only root reads it (from §18.6, a process with `CAP_SYSLOG`), which is Linux's behaviour with `kernel.dmesg_restrict` at 1, since the log can carry kernel addresses; a read as uid 1000 fails with `EPERM`. Each read returns one record in Linux's `/dev/kmsg` form (`<prio>,<seq>,<usec>,-;<text>`); `log::Record` gains a `u64` sequence number, the ring's count of records written before it (DESIGN §2.5), which the §10.7 core tool reads through the shared type; a read whose next record the ring has overwritten returns `EPIPE` once and resumes at the oldest record, as Linux's does. An in-guest test lets the ring overwrite a stalled reader's next record, gets `EPIPE`, and then reads consecutive sequence numbers
...
- [ ] `procfs` backed by the process table: `cmdline`, `status`, `maps`, `fd`, `stat` per pid, each in Linux's format, and vibeOS's own counts in `/proc/<pid>/vibeos/faults` and `/proc/<pid>/vibeos/syscalls` (§12.2's and §10.7's) and `/proc/vibeos/faults` (the global fault counts), each with a `docs/LINUX.md` native row; the §10.5 `/bin/sh` `ps` built-in reads it and syscall 500 is deleted; a file that reads another process's address space (`maps`) pins it with a get-unless-zero `users` reference (DESIGN §2.11) for the read only
...
- [ ] `timer_create`, `timer_settime`, `timer_gettime`, and `timer_delete` with `SIGEV_SIGNAL` and `SIGEV_THREAD_ID` (musl builds `SIGEV_THREAD` on it), and `setitimer` and `getitimer`, with `alarm` as an x86_64 entry point onto `setitimer`, on one timer queue shared with §13.6's `timerfd`, whose expiries run as DESIGN §2.2 timer callbacks; a deadline set as an absolute time on `CLOCK_REALTIME` (`clock_nanosleep` and `timer_settime` with `TIMER_ABSTIME`, `timerfd_settime` with `TFD_TIMER_ABSTIME`, and a §13.5 wait with `FUTEX_CLOCK_REALTIME`) expires when the realtime clock reaches it, so a `clock_settime` or `settimeofday` that steps the clock re-arms every such deadline, as Linux does (DESIGN §6.6); a relative deadline, and a deadline on any other clock, does not move. In-guest, as root: a futex wait, a `clock_nanosleep`, and a timerfd, each with an absolute `CLOCK_REALTIME` deadline 60 s ahead, end within 1 s of a `clock_settime` that steps the clock 120 s forward, while a relative 2 s `clock_nanosleep` on `CLOCK_REALTIME` begun before the step still sleeps at least 2 s
- [ ] `ioctl` with a registry rather than a growing match arm; a request no handler of that file registers returns `ENOTTY`, as Linux's does
...
- [ ] a generator in the user crate emits syscall sequences over this phase's calls from the §10.5 table: files, directories, links, renames, descriptors and offsets, pipes, `fork` and `wait4`, and signals with default actions, sized within vibefs v1's limits (VIBEFS.md §3) until §14.8. The same static binary runs each sequence under the oracle in a tmpfs directory and in the guest on vibefs and on tmpfs, recording every return value, errno, and the final file tree; sequences also pass numbers with their high 32 bits set, 64-bit values in 32-bit arguments, and two bad arguments at once, so SYSCALL.md §1's number, width, and order rules are checked against Linux
...
- [ ] a static Rust `std` program, built with the repository's pinned Rust toolchain for each `*-unknown-linux-musl` target, whose threads each block for 1 s in `Mutex::lock` while another thread holds the lock, in `Condvar::wait_timeout`, and in `thread::park_timeout`, and which fails when a thread's CPU time across its waits, by `clock_gettime(CLOCK_THREAD_CPUTIME_ID)`, exceeds 10 ms; it checks §13.5's bitset operations as `rustc` and `cargo` (§17.7) call them
- [ ] the corpus, the generator, busybox's suite, LTP, the Go program, and the Rust program run on the nightly job on both architectures
...
- [ ] the §13.5 futex table, wait, wake, and requeue in the portable half with a loom model (§10.8): a wake racing a timeout, a requeue racing a wake, a bitset wake that matches only some of a queue's waiters racing a plain wake, a waiter whose page breaks COW, and a waiter whose word read faults, drops the bucket lock, and retries while a wake runs; a mutant that checks the futex word outside the bucket lock must fail the model
...
- [ ] the §10.5 table's C-type column gains an argument kind per parameter: descriptor, user pointer with the index of its length argument, path, flag set with its valid mask and whether the call rejects an unknown bit with `EINVAL` or ignores it, as Linux's call does (the mask acts only on a row that rejects), pid, signal, or plain integer; the fuzzer generates from it and the §9.3 syscall trace decodes with it
...
- [ ] vibefs format version 2, designed for the §12.5 page cache: multi-GiB volumes, at least a million inodes, B-tree directories without a volume-wide entry cap, and extent trees rather than four extents per inode, which `write` fills with multi-block extents; VIBEFS.md gains the v2 format before any code, as §8.5 did for v1, and it meets every requirement of VIBEFS.md §15: host tests of the shared format code write and read back a block number above 2^32, an inode number above 2^32, a 255-byte name, and a nanosecond timestamp, check CRC-32C against its published vectors, read one 4 KiB block of a 1 GiB extent with exactly one data-block read, overwrite one block in the middle of a 16-block extent and find the other 15 blocks' checksums unchanged, and refuse a metadata block whose header names another `meta_uuid`, another block number, or an unknown incompat feature; and refuse a metadata block whose generation or checksum differs from the pointer that reached it: over a `Disk` that acknowledges a metadata write and the flush after it but keeps the block's older bytes, and over one on which a commit whose superblock never became durable is followed, after a remount, by a commit of the same number that reuses one of its addresses and whose write there is dropped, a read that reaches that block returns `Corrupt`, `fsck-vibefs` reports the generation the pointer expects and the one the block holds, and no stale subtree is presented as part of a committed generation; and `tune-vibefs --uuid`, a CLI over the same code, gives a volume a new `uuid` by writing its two superblock slots and no other block, after which the volume mounts and `fsck-vibefs` finds it clean with its `meta_uuid` unchanged; and a volume whose reserved intent-log inode (VIBEFS.md §15's Commit row) holds extents mounts and `fsck`s clean, no directory lists that inode, and commits allocate none of its blocks; a read loads the checksums of the blocks it reads before it submits them, and a host test reads a block whose checksum-tree leaf is not cached and counts no metadata read issued from its completion (F051)
- [ ] v2 superblocks as VIBEFS.md §15's Superblocks and geometry row places them, with host tests of the shared format code: a 64 KiB-block volume's superblocks are found at the same byte offsets as a 4 KiB-block volume's; a volume whose first 64 KiB is overwritten mounts; a torn newest superblock, and a newest superblock whose inode-tree root was never written (a `Disk` that acknowledges that write and its flush and keeps the old bytes), each mount the older generation, the second with an error-level log line naming both generations, the rejected generation recorded for `fsck-vibefs`, and the next commit numbered above it; after every commit of a workload of creates, overwrites, and unlinks, the older slot's tree passes `fsck-vibefs`'s checks; the free space `statfs` reports leaves out the blocks the last commit freed; and an image on an in-memory sparse `Disk`, grown from below 2^32 blocks to above it by a commit and remounted, passes `fsck-vibefs`. A workload in which the held-back blocks exceed one commit's frees, or a fallback that finds the older slot's tree damaged, reopens the Superblocks and geometry row
- [ ] the v2 inode model of VIBEFS.md §15's File kinds, Link counts, Orphans, Inode flags, Extents, NOCOW files, and Directories rows, with a host test of the shared format code for each: each of the seven kinds survives a remount, among them a device inode whose major and minor are each above 2^16; a directory with three subdirectories has `nlink` 5, and 4 after one is removed; an unlinked file still open and an `O_TMPFILE` inode each survive a commit and, after a simulated crash, are freed with their blocks by the next read-write mount, while an `O_TMPFILE` inode that `linkat` named survives it; each defined inode flag and a project id survive a remount, and an unknown flag bit is refused; a range reserved past the end of a file reads as zeros and a punched hole frees its blocks; a NOCOW file keeps its block numbers across overwrites, and after a snapshot its first write to a block moves that block once; the swap check accepts a NOCOW file whose every block has an extent and no snapshot shares one, and refuses one with a hole, one with a block a snapshot shares, and one without the NOCOW flag; and while 10,000 random adds and removes run, a reader that resumes from every cookie it was given sees each entry that stayed in the directory exactly once and never a cookie at or above 2^63. A row whose test here, or §14.9's tarball build with device nodes, FIFOs, and hard links, cannot pass on the chosen record layout reopens that row before any v2 volume ships
...
- [ ] v2 snapshots as VIBEFS.md §15's Snapshots row specifies: created, listed, and deleted through ioctls on `/dev/vibeos/vibefs-control`, which `docs/LINUX.md` lists as a native interface, and mounted read-only with `snapshot=<name>` beside the live tree; `fsck-vibefs` walks every snapshot tree and checks every deadlist (VIBEFS.md §11), and the §12.5 workload that creates and deletes a snapshot runs on them. Host tests on the shared format code: creating a snapshot writes the same number of blocks on a volume with 100 used blocks as on one with 20,000; overwriting blocks under a snapshot and then deleting it frees exactly the overwritten blocks; deleting the middle one of three snapshots leaves the other two reading back their contents and `fsck` clean; deleting a snapshot whose deadlist is empty reads and writes the same number of blocks on both volumes; and a deadlist entry that the snapshot's successor still reaches, planted in an image, is an `fsck` error. In-guest, a mounted snapshot reports an `st_dev` other than the live mount's and the live tree's inode numbers, and a write to it fails with `EROFS`. The host tests count the blocks each create and delete touches: deadlist processing on delete that costs more than v1's eager walk at the snapshot counts §12.5's workloads use, or a later line that needs writable per-directory snapshots (btrfs's subvolumes), reopens the Snapshots row
...
- [ ] `mkfs-vibefs --from <tarball>` builds a vibefs v2 image from a root filesystem archive on the host, taking owners, modes, device nodes, FIFOs, symlinks, and hard links from the archive headers (VIBEFS.md §15's File kinds and Link counts rows), so no host needs root or a case-sensitive filesystem to build it
...
### 15.1 netdev layer
- [ ] a `NetDevice` trait: transmit, MTU, MAC, link state, and statistics; an interface registers as a DESIGN §12 device with an ifindex allocated in increasing order and not reused at once, and unregistering it deletes its routes and neighbour entries, takes it out of any bridge (§21.7), fails sends on sockets bound to it with `ENODEV`, and frees it at its last reference (DESIGN §12.4); a host test unregisters an interface that a route and a bound socket use, and the route is gone and the socket's next send returns `ENODEV`
...
- [ ] filesystem behavior compilers depend on: `rename` atomicity, `O_TMPFILE` (on vibefs v2 through VIBEFS.md §15's Orphans row), `fsync` semantics, correct `mtime`
...
- [ ] tracer writes stay in user space: `POKEUSER` of a debug-register address, and `SETREGSET` of `NT_ARM_HW_BREAK` or `NT_ARM_HW_WATCH`, refuse an address at or above `USER_MAP_END` with `EINVAL`, and `SETREGS` or `POKEUSER` of an `fs_base` or `gs_base` at or above it returns `EIO`, as Linux does at `TASK_SIZE_MAX`; in-guest, a tracer's `SETREGS` with CS 0x08 gets `EIO`, and one with CS 0x0B (the kernel code descriptor at RPL 3) ends the tracee with `SIGSEGV` when it resumes; a `SETREGSET` of `NT_PRSTATUS` whose `pstate` is EL1h (0x5) or sets DAIF.I gets `EINVAL`; and in each case the kernel keeps running
...
- [ ] a translation fault reported with the device, address, and access kind, counted per device, and passed to the device's error handler (DESIGN §10.3), which resets it rather than hanging
- [ ] the virtio transport checks each used-ring id: an id at or above the queue size, or one that is not the head of a chain the driver posted (checked against a driver-private shadow of each head), wakes the device's error handler (DESIGN §10.3), which resets the device, instead of freeing a chain, and each harvest pass handles at most queue-size entries; host tests drive the §6.5 simulated device with duplicate, non-head, and out-of-range ids (F048)
...
- [ ] `CR4.FSGSBASE` enabled. Once a user can `wrgsbase` a kernel-half value, the §10.6 sign check is unsound, so an NMI, `#MC`, or `#DB` taken at CPL 0, and every `#DF`, saves `GS_BASE` with `rdgsbase`, loads this CPU's `PerCpu` pointer (kept at the top of each per-CPU IST stack), and restores the saved value on exit; one taken at CPL 3 keeps §10.6's CS.RPL `swapgs` and move to the thread's kernel stack (DESIGN §5.10 rule 3), so a user thread's GS base is in `KERNEL_GS_BASE` whenever the thread is in the kernel. Every switch away from a user thread reads back the live user FS base and user GS base (the latter from `KERNEL_GS_BASE` while in the kernel), since `wrfsbase` and `wrgsbase` change them without a syscall. `CR4.FSGSBASE` is set only once the `lfence` box above has landed (F133)
...
- [ ] mount namespaces through `unshare(CLONE_NEWNS)` and `clone(CLONE_NEWNS)`, and `pivot_root`, as Linux defines them, with `mount`'s `MS_BIND` and `MS_BIND | MS_REC`, which container runtimes use to make the new root a mount point before `pivot_root` (moved from §23.5); `setns`, `/proc/<pid>/ns/*`, and the other namespaces are §21.5. In-guest: a directory bind-mounted at a second path shows the same entries through both, and a file renamed through one is found under its new name through the other; after `pivot_root` in a new mount namespace, a plain `umount2` of `put_old` succeeds while a process in the parent namespace holds a file open on the same volume
...
- [ ] full disk encryption with AES-XTS, added to the §14.7 facade over the `xts-mode` crate and RustCrypto's `aes`, with the IEEE 1619 vectors as host tests, in LUKS2, the format cryptsetup writes, so a Linux system can open a vibeOS disk and one format serves from here to §29.1's cryptsetup: `docs/LUKS2.md` documents the two binary headers and their checksums, the JSON metadata, keyslots, the anti-forensic splitter, digests, and segments from the published LUKS2 on-disk format specification before any code, cited and not copied (DESIGN §1.5), as §29.2 documents md 1.2. A keyslot's key is derived from a passphrase with §14.7's Argon2id, and PBKDF2 with HMAC-SHA-256, LUKS2's volume-key digest, joins the facade through RustCrypto's `pbkdf2`, with the RFC 7914 vectors. The kernel never parses the header (DESIGN §2.10): the transform is a device-mapper `crypt` target (`aes-xts-plain64`, with its `sector_size` option) stacked on any §7.1 `BlockDevice`, created, loaded with a cipher, a key, and an offset, activated, and removed through the calls of Linux's device-mapper interface on `/dev/mapper/control` that the unlock tool below issues (`DM_VERSION`, `DM_DEV_CREATE`, `DM_TABLE_LOAD`, `DM_DEV_SUSPEND`, and `DM_DEV_REMOVE`), in Linux's layouts, with the mapped device at `/dev/dm-<N>` and `/dev/mapper/<name>`; its decryption on read runs in stage 2 (DESIGN §10.1); §29.1 extends that interface and adds no second key path. A native tool in a portable crate formats a volume (`aes-xts-plain64` with a 512-bit key, Argon2id keyslots, 4096-byte sectors), opens one from the initrd by asking its passphrase and loading the table, changes a passphrase, and adds and removes keyslots; it opens any LUKS2 volume whose cipher, sector size, key derivation, and digest it implements, cryptsetup's defaults among them, and refuses any other with a named reason. Host tests open, with the tool, volumes that cryptsetup created under Linux; and in a job on the hosted Linux runner, cryptsetup prints the header of a volume the tool formatted (`luksDump`) and accepts its passphrase (`open --test-passphrase`). A LUKS2 feature that cryptsetup's defaults use and the tool cannot implement from the published specification reopens this choice before Phase 22 ships an installed disk
...
### 19.3 Benchmarks
- [ ] microbenchmarks: syscall latency, context switch (between two user threads that hold live FP state, and between a user thread and a kernel thread, so the FP binding's save and deferred load are both measured), page fault, allocation, lock acquire; if the binding's check and deferred load cost more than the reloads they skip, that regression reopens loading at switch-in (DESIGN §7.5)
...
- [ ] load balancing: periodic first, then work stealing, measured against the periodic baseline before keeping it; it moves preempted threads, which DESIGN §2.9 rule 5 makes safe, and §10.3's `current` migration test also runs with the balancer moving threads in place of its hook
...
- [ ] one kernel log store and one printer thread per console, as DESIGN §2.5's log contract specifies, in place of the global IRQ-safe ring, its TAS lock, the per-CPU serial capture stage, and the serial try-lock sink, so an emitting CPU does not wait on the UART: a lockless ring of fixed-size slots with a state word each, whose writers take a sequence number from one `fetch_add` counter, claim their slot by compare-and-swap, and commit with a Release store, so NMI and `#MC` handlers may append; the sequence numbers continue §13.9's `/dev/kmsg` numbering; `marker!` still writes serial before it returns; records print synchronously until a console's printer runs and, on the dump owner only, after `HALTING`. A loom model beside §10.8's runs three writers, one of which runs between another's claim and commit as an NMI would, against a reader and a printer, and its weakened variant, with the commit store Relaxed, must fail; §11.7's litmus set gains the commit and the read. An in-guest test at `-smp 4` has every CPU emit 10,000 `klog!` lines while a `/dev/kmsg` reader follows: each sequence number it reads is one more than the last or follows an `EPIPE`, and the serial printer's `vibeOS: log: N records dropped` lines account for every gap. The §10.7 `#[repr(C)]` types and the core tool change in the same commit, DESIGN §2.3, §2.5, and §7.7 describe the store as built, and contention on the sequence counter is measured before and after with this subsection's other numbers; if the counter's cache line is among the most contended, or Phase 27's 1024-vCPU run shows it to be, the slots go per CPU while the one counter still orders them, and this box records the numbers; this closes §5.5's deferred box
...
- [ ] idle through `mwait` on x86_64 where CPUID reports MONITOR, falling back to `hlt`, and `wfi` on aarch64 with IRQs masked (DESIGN §11.1's idle row), all at the shallowest state, which needs no firmware tables; deeper C-states and frequency scaling from ACPI are §20.2's, and measured power needs a physical machine ([Funded goals](#funded-goals))
...
- [ ] `sendfile` and `splice`
- [ ] direct I/O bypassing the page cache; a direct-I/O write to a file whose data is checksummed copies the user buffer into a kernel buffer and checksums the copy (DESIGN §10.6), and in-guest an `O_DIRECT` writer that changes its buffer while each of 1,000 writes is in flight never makes a later read of the file return `Corrupt`
- [ ] user pages held by a device (direct I/O, the zero-copy send path, registered `io_uring` buffers) are pinned, with a pin count in the §12.1 unit's head `Frame`. A pinned anonymous page is always exclusive to one address space: pinning a COW-shared page breaks the share first, and `fork` copies a pinned page eagerly instead of sharing it, so the §12.3 write fault reuses a pinned page instead of copying it. Reclaim and §19.7 migration skip pinned pages. A pinned page of a file whose data is checksummed is written back from a copy, checksummed over the copy (DESIGN §10.6)
...
- [ ] the stage-2 hop measured (DESIGN §10.1): 4 KiB random reads of an uncached vibefs v2 file at queue depths 1 and 32, with data-checksum verification run as a stage-2 item and, in a measurement build, inline in the bottom half, under KVM on the KVM runner; the 50th and 99th percentile latencies are recorded in `docs/`. If inline is more than 10% faster at the 99th percentile, DESIGN §10.1 lets verification and decryption, which submit no I/O, run in stage 1, and the rule that no stage waits stays
- [ ] boot time reduced by parallelizing device probing and deferring what can be deferred
- [ ] virtio-blk zero-copy: DMA directly from page-cache pages once §12.5 unifies the caches; drop the bounce path; §12.5's stable-page test runs again with the bounce path gone
...
- [ ] under QEMU on both architectures (TCG, 2 vCPUs, 1 GiB), a virtio-blk disk added with `device_add` on a PCIe root port during a write workload appears under its persistent name, and one removed with `device_del` fails its in-flight I/O without a panic, through native hotplug (the §20.9 `q35` variant and aarch64 `virt`) and through ACPI hotplug on `q35`; when the removed disk holds a mounted vibefs volume with a file open, a `MAP_SHARED` mapping of that file, dirty pages, and the disk's block node open, reads of the file return `EIO`, a fault on an unpopulated page raises `SIGBUS`, the next `fsync` returns `EIO`, a read of the block node returns no data, `umount2` with `MNT_DETACH` succeeds, and after the last close the frame count and the block registry are back at their baseline; a variable written through efivarfs reads back after a reboot under OVMF and the aarch64 edk2 build
...
- [ ] a panic record, the §5.6 dump and the last log records, kept with a header holding a magic and a format version, a sequence number, and a checksum, the one format §25.6's firmware stores also use (DESIGN §2.5), in a RAM region at a physical address fixed by `vibeos.panic_record=` on the §10.2 command line, which the frame allocator never hands out; at boot the region is checked against the Limine memory map and module placement, a conflict is logged rather than trusted, and a region whose header or checksum fails reads as empty. It is read back, logged into the kernel log, and cleared on the next boot after a warm reset, since there is no host to catch it; tested under QEMU with a deliberate panic and the monitor's `system_reset` on both architectures, after which a second `system_reset` logs no record. Firmware-backed stores and full memory dumps are Phase 25
...
- [ ] `suspend` and `resume` on the `Driver` trait (§6.1), called in DESIGN §12.2's per-device order, a device's children and consumers suspending before it and its parent and suppliers resuming before it, which S3 needs; a block driver's `suspend` begins with DESIGN §10.3's queue quiesce
...
- [ ] removal: a USB device detached with `device_del` during a transfer fails its I/O, its nodes go away, and nothing panics; a read from an open `/dev/input/event<N>` of a detached `usb-kbd` returns `ENODEV`, and a detached `usb-storage` disk holding a mounted FAT volume with a file open behaves as the Phase 20 gate's virtio-blk disk does: reads of the file return `EIO`, `umount2` with `MNT_DETACH` succeeds, and after the last close the block registry is back at its baseline (DESIGN §12.4)
...
- [ ] the §12.5 error handling (DESIGN §10.3), one error handler per controller: a timed-out command is aborted with the admin Abort command, whose wait is the command's deadline, as Linux gives an aborted command a second timeout, and the controller is reset (`CC.EN` cleared, `CSTS.RDY` read back as 0) when the abort does not complete, and I/O resumes on the reset controller; the driver's reset bound R is that wait plus `CAP.TO`; tested with a `kernel_tests` hook that withholds one completion, and with one that posts the withheld command's completion just after its Abort is sent, which completes the command once and resets nothing
...
- [ ] the §12.5 error handling (DESIGN §10.3), one error handler per port: a timed-out command is recovered by a port reset (COMRESET), after which the port's other commands are reissued; the driver's reset bound R is its port reset's bound, recorded in the box's commit; tested with a `kernel_tests` hook that withholds one completion
...
- [ ] ACPI where firmware gives no device tree, as servers and `sbsa-ref` do, filling §11.5's `MachineDesc` as a second producer beside the device tree, so none of its consumers changes: the §2.4 parser for the FADT's Arm boot-architecture flags (`PSCI_COMPLIANT`, `PSCI_USE_HVC`), which say whether PSCI is present and name its conduit where there is no `/psci` node, and for MADT, GTDT, SPCR, MCFG, IORT (the ITS device IDs behind each root complex, the §18.1 SMMUv3, and its RMR nodes, whose ranges are mapped identity into the domains of the stream IDs they name, as §18.1 maps RMRRs and IVMD blocks, with those stream IDs left in bypass from SMMU enable until their domain attaches; QEMU generates no RMR node, so those are host-tested against IORT tables the pinned `iasl` compiles), SRAT (§19.7's NUMA code path on aarch64, tested with `-numa` on `virt`), and TPM2, so §18.7's driver finds `tpm-tis-device` on the ACPI `virt` profile as it does on x86_64, and the §20.2 interpreter for the DSDT, whose `_CCA`, with IORT's coherency attribute for each root complex and named component, gives each device its DMA coherence (DESIGN §4.7)
...
- [ ] Limine's EFI system table and EFI memory map responses captured in `BootInfo`; the runtime regions mapped in a page table of their own, with the choice between `SetVirtualAddressMap` and physical-mode calls recorded in DESIGN; calls made one at a time with interrupts off, on both architectures; around each call the live user FP and SIMD state is saved and this CPU's FP owner emptied (DESIGN §7.5), so the next return to user mode loads saved state, since firmware may use those registers and the kernel is soft-float (§11.1, §11.6); no FP or SIMD access traps, so none needs enabling
...
- [ ] removal kills the device in DESIGN §12.4's order (§2.11 rule 3): the device is unpublished and its gate closed, so I/O through a handle still held fails with `Gone` and no open descriptor or mount on the device delays the removal; in-flight I/O completes or fails; the driver's `remove` stops the device and frees its vectors; its IOMMU domain is detached and the IOTLB and device TLB invalidated before any DMA buffer is freed; and nothing panics. Removing a bridge or root port removes the devices below it first, deepest first (DESIGN §12.2). A surprise removal takes the same path after the slot's presence-detect or link-down interrupt marks the function disconnected, host-tested against a simulated slot whose presence detect drops without an attention-button press. In-guest on the §18.1 `q35` configuration with the IOMMU on, the removed disk's DMA frames return to the buddy allocator only after its domain is detached and the invalidation has completed; a buffer that a failed stop left quarantined (DESIGN §10.3) is freed once presence detect drops, and the surprise-removal host test finds the frame count back at its baseline
...
- [ ] the KVM selftests (`tools/testing/selftests/kvm`) from the source tarball of the §12.4 oracle kernel, built static on the host as test inputs and never shipped, run in the vibeOS L1 and in §21.3's Linux L1 in the same job, in the nested job and the EL2 job: every case that passes under the Linux L1 passes under vibeOS, skips for a capability `docs/LINUX.md` lists as absent, or is on a checked-in expected-failure list per architecture in §13.11's format
- [ ] kvm-unit-tests at a pinned release, built on the host, each test run by its `run_tests.sh` with `ACCEL=kvm` under the §21.2 VMM and under the same QEMU in §21.3's Linux L1: vibeOS passes at least 90% of the tests that pass under the Linux L1, per architecture and, on x86_64, per vendor path, on the weekly job set up as the nested job on x86_64, with each path §20.8's records show offered among the last two weeks' runs, and in the HVF record on aarch64; the weekly job set up as the EL2 job records both counts
- [ ] L2 guests get virtio block and network devices whose sequential throughput is at least half of what the same L2 guest gets under §21.3's Linux L1, on the harness's default machine (`pc` on x86_64, `virt` on aarch64) with virtio-net on a tap with `vhost=on` under both L1s, in the nested job and the HVF record; the EL2 job records both numbers
...
- [ ] EPT or nested paging for guest physical to host physical translation, invalidated with the host PTEs behind them as DESIGN §2.4 orders (§21.2)
...
- [ ] the KVM ABI is the ioctls and capabilities `docs/LINUX.md`'s KVM section lists, each behaving as Linux's `Documentation/virt/kvm/api.rst` defines it at the baseline (How to read this): `KVM_CHECK_EXTENSION` returns 0 for every other capability; `KVM_GET_SUPPORTED_CPUID`, `KVM_GET_MSR_INDEX_LIST`, and `KVM_GET_MSR_FEATURE_INDEX_LIST` on x86_64, and `KVM_GET_REG_LIST` on aarch64, report only the CPUID bits, MSRs, and registers the hypervisor virtualizes; and an ioctl outside the list fails as Linux fails one its descriptor does not know. The first list is what the pinned QEMU uses: the ioctls it issues and the capabilities it queries, captured with `strace -f` under §21.3's Linux L1 while it runs this phase's gate guests in the nested job and the EL2 job, and checked in with the QEMU version, each capability marked implemented or absent. When the pinned QEMU changes, those jobs retake the capture and fail on an ioctl or a capability the list does not name, so the bump lands with its implementation or an absent entry
- [ ] guest memory as an address space, with host page faults servicing guest access. Every change or removal of a host PTE that backs guest memory invalidates the VM's second-level entries for that range on every CPU that may hold them before the frame's count drops, inside §12.3's gather, shoot down, then release path, and a second-level fault that races an invalidation retries (DESIGN §2.4). In the nested job and the EL2 job, with the vibeOS host on the §12.1 KASAN build: a vibeOS L2 guest writes a pattern into pages, gives them to its §21.4 balloon so that QEMU discards them, deflates, and reads zeros from them; and with the L2 guest's RAM a `memory-backend-file` with `share=on` on a vibefs file, the host fills its page cache until §12.6 reclaim drops the guest's clean pages, and the guest reads its pattern back; the host reports nothing
...
- [ ] `vhost-net` and `vhost-vsock` in the kernel, so QEMU's virtio devices reach the gate's throughput without a userspace hop; a vhost worker holds a reference to the VMM's address space from `VHOST_SET_OWNER` until its device is released and reaches guest memory only through the §10.6 accessors, never through a frame or kernel mapping it caches (DESIGN §2.4)
- [ ] vhost behind a guest IOMMU, as QEMU drives it for a device started with `iommu_platform=on` (§18.1): `vhost-net` and `vhost-vsock` offer `VIRTIO_F_ACCESS_PLATFORM` in `VHOST_GET_FEATURES` and `VHOST_BACKEND_F_IOTLB_MSG_V2` in `VHOST_GET_BACKEND_FEATURES`, accept it through `VHOST_SET_BACKEND_FEATURES`, and translate each guest I/O address through a per-device IOTLB that Linux's device-IOTLB messages on the vhost descriptor fill, in `struct vhost_msg_v2` as `include/uapi/linux/vhost_types.h` defines it: for an address with no entry, vhost queues a `VHOST_IOTLB_MISS` that QEMU reads, and the virtqueue waits for the answer; QEMU writes `VHOST_IOTLB_UPDATE` and `VHOST_IOTLB_INVALIDATE`, and an invalidation ends the device's use of the range before the write returns. A translated address lies in the VMM's address space and is reached as the box above says. QEMU refuses a `vhost=on` device with `iommu_platform=on` whose backend does not offer `VIRTIO_F_ACCESS_PLATFORM`. In-guest: an L2 guest with virtio-net on a tap, `vhost=on`, and `iommu_platform=on` carries TCP to a peer in the L1 with every byte checked, in the nested job on §18.1's `q35` configuration with `intel-iommu`, and in the EL2 job on §18.1's `virt` configuration with `iommu=smmuv3`
...
- [ ] an update installs unattended and takes effect on the next boot; three bad updates, one that panics, one that hangs with interrupts off, and one that fails its health check, each come back on the old root with no human action; a good update whose first trial the harness resets before the trial's root is mounted is armed again and commits at the next boot, and a bad update that hangs before its root is mounted is armed again at the next two boots and then recorded as a hang before the root; a user and their password, a file in `/home`, a changed `/etc` file, and a package the user installed survive each good update and each rollback, and an `/etc` file the user never changed holds the running release's default after each; and a power cut at each of 20 seeded writes of an update, on §10.2's volatile-cache device, leaves the old slot booting with an `fsck`-clean ESP, on both architectures under QEMU with OVMF or edk2 and a writable variable store (TCG, 2 vCPUs, 2 GiB)
...
- [ ] versioning with a defined policy for what constitutes a break: vibeOS's own formats and native interfaces break only in a major version, while the Linux surface is versioned by its baseline (§39.1), so a change that brings it closer to the baseline is a fix and a move to a later baseline is a minor release; the policy covers rollback: a release never leaves the §22.2 state partition unreadable to the release in the other slot. It never sets an incompat or ro_compat feature bit on an existing vibefs volume on its own, the intent log's `log_replay` bit, which a volume carries only while its log may hold records, excepted (VIBEFS.md §14); enabling such a feature is an admin command that names the oldest release still able to mount the volume, and `mkfs` uses only features the oldest supported release reads. Every other format the base system keeps on the state partition (the `/etc` files it parses, the requested-package list, the `/etc` checksum record, the key-set record, and the updater's records) changes the same way: a release writes a new format or field there only once the oldest supported release reads it
- [ ] formats kept from Phase 22's release, the first the §22.2 installer ships: a release reads what any earlier release of its major version wrote in the formats below, and, since a rollback from the firmware menu or a kexec to an older kernel runs older code on newer data, every change to them is an addition an older reader ignores or a new version written beside the old one; the unattended-install configuration keeps only the first rule, since only a later installer reads an earlier one. The formats: vibefs v2, through VIBEFS.md §15's feature flags, under the rule above on setting them; the §14.6 package, repository, release-manifest, and key-set record formats; the §22.2 disk layout (each partition's GPT type and label, and the ESP's slot directories) and unattended-install configuration; the §18.7 LUKS2 header, which stays LUKS2 as its published specification defines it, and whose existing keyslots, KDFs, and tokens a release converts only when an administrator asks, since the release in the other slot may not read the new type; the §14.3 service definition; the shadow file's §14.7 password-hash string, which carries its algorithm and parameters; and the §20.1 panic record's header, format version, and checksum, and where each store keeps it, but not the dump and log records inside it. Each is documented in `docs/`. A later phase that adds a format one release writes and another reads adds it to this list in the same commit. Phase 22's release is the first release of major 0: every later release of major 0 updates an install of it (§22.2)
- [ ] a format corpus from Phase 22's release on: the release workflow keeps as release assets, per architecture, a sample of each format the box above lists that the release writes (a vibefs v2 image, its signed repository and packages, a small LUKS2 volume and its passphrase, a set of service definitions, an unattended-install configuration, a shadow file and its passwords, and a panic record from §20.1's deliberate-panic test) and a FAT image; a nightly job reads every kept release's corpus with `main`: it mounts, reads, writes, and `fsck`s the images, installs, upgrades, and removes the packages from the kept repository, unlocks the volume, verifies each shadow entry, and parses the rest with the code that reads them. The disk layout is read by §22.2's update test from Phase 22's release. A format added to the list adds its sample in the same commit
...
- [ ] user creation, locale, timezone, network configuration, written to the state partition's `/etc`
- [ ] an unattended install from a configuration file, which is how CI installs it
- [ ] upgrades between releases through the A/B update below, tested from every supported prior version and from Phase 22's release, the first the installer ships (§22.1)
...
- [ ] a state partition that both slots mount: the installer creates, beside the ESP and the two slot roots, one vibefs v2 partition labelled `vibeos-state`, found by that label only on the boot disk, as a slot's root is. `/etc`, `/home`, `/root`, `/srv`, and `/var` live on it, and `/opt` and `/usr/local` point into `/var`. A slot root holds only what its packages own, the database of those packages included, and is read-only once the update that wrote it has finished. Packages ship their default `/etc` files under `/usr/share/factory/etc`. At every boot, before init reads any configuration, each default of the running slot is copied into `/etc` where the file is missing or still holds the default copied last, judged by a checksum the state partition records; a file the user changed gets the new default beside it as `<file>.new`, with a log line; and an unchanged file the running slot no longer ships is removed. So a failed trial or a rollback leaves the running release's defaults in `/etc`. A package the user installs or removes between updates is staged into the inactive slot as an update is and takes effect through the same trial boot; the state partition keeps the list of packages the user asked for, which each update installs with the release's packages. The updater's records, the key-set record it trusts (§14.6), and §14.9's `vibeos-linux` roots live in `/var`, so a rollback never brings back a revoked key and an Alpine root stays writable. If the boot-time `/etc` refresh proves too costly for Phase 22, the fallback is `/etc` wholly on the state partition with no factory refresh, recorded in this box, not dropped
- [ ] an update writes the ESP only inside the inactive slot's directory and never rewrites the active slot's files: it writes each new file and flushes it before any directory entry names it, and the §8.3 FAT driver writes a changed FAT sector to the first FAT copy, flushes, and only then writes the second copy, so a power cut leaves at most one copy mid-write, and the boot-time `fsck` of the recovery box repairs it from the other. A host test cuts power at each write of an update to a FAT image on a `Disk` that tears a write at any 512-byte boundary, and after the repair the active slot's files read back intact every time
- [ ] a trial boot: the updated slot boots once through UEFI `BootNext` (§20.9), and is committed by rewriting `BootOrder` only when a declarative health check passes (services up under §14.3's init, the network reachable, and a probe each service defines); a failed check reboots; the check's deadline is at least half the watchdog timeout init reads with `WDIOC_GETTIMEOUT`, so at least the largest stall bound S among the block devices (DESIGN §10.3), and a disk that stalls and recovers does not fail a good update
...
- [ ] §20.6's `i6300esb` driver armed at boot and fed, on an encrypted install, first by the initrd's unlock program, from a loop that waits on no input or disk while the prompt waits for the passphrase and while the root is mounted, and then by init, to which it hands the open descriptor, for as long as the system runs, so a hang during a trial boot, or after one, resets the machine; the watchdog core keeps the timeout at or above twice the largest stall bound S among registered block devices (DESIGN §10.3), raised when a device with a larger S registers: a `WDIOC_SETTIMEOUT` below that floor sets the floor and returns it, as a Linux driver returns the timeout it could set, and `docs/LINUX.md` records the floor as a deliberate difference; where the hardware's maximum is below the timeout, the core feeds the hardware in between, as Linux's watchdog core does. Init locks its memory with `mlockall` (§12.4) and feeds from a loop that makes no filesystem call, so a storage stall alone never starves the watchdog
- [ ] a trial boot that panics, hangs, or fails its health check comes back on the old root at the next reset with no human action, since `BootNext` lasts one boot; the state partition keeps what the trial wrote, and the old release's boot puts its own defaults back in `/etc` (the state-partition box); Phase 25 extends the panic policy and the watchdogs
- [ ] a trial that never reached its root is not a failed update: the trial writes a record on the state partition once its root and the state partition are mounted, and names the failing probe there before a failed health check reboots. When the old slot boots and finds the trial it armed uncommitted, it records the update as failed, with its cause, if the trial reached its root or left a panic record (§20.1): a panic from the panic record, the health check from the trial's record, and otherwise a hang. A trial that ended before its root with no recorded cause, a reset or power cut at the passphrase prompt or before it, is armed again for the next reboot, at most twice, and then recorded as a hang before the root. On an encrypted install every boot asks for the §18.7 passphrase, trial boots included: an unattended update installs and arms its trial without a human, and the trial runs when someone unlocks it, while the prompt feeds the watchdog (above)
...
- [ ] `clone3` over the same code as `clone`, taking every flag `clone` takes, plus `CLONE_PIDFD`, since glibc's `pthread_create` and `posix_spawn` try it first and fall back to `clone` only on `ENOSYS`; `clone_args` is read through §13.10's extensible-struct helper
...
- [ ] `close_range`, `copy_file_range`, `statx`, `faccessat2`, `openat2` (its `open_how` read through §13.10's extensible-struct helper), and `epoll_pwait2`, which glibc, coreutils, libuv, and Rust's `std` call before any fallback; on vibefs, `copy_file_range` copies bytes until the clone feature in [Beyond](#beyond) (VIBEFS.md §15)
...
- [ ] `fallocate` (with `FALLOC_FL_KEEP_SIZE` and `FALLOC_FL_PUNCH_HOLE`), `posix_fadvise`, `readahead`, `sync_file_range`, and `syncfs` on vibefs v2 and tmpfs, as PostgreSQL and `dpkg` call them; on vibefs v2, `fallocate` records unwritten extents and a punched hole frees its blocks, and `lseek` takes `SEEK_DATA` and `SEEK_HOLE` over holes and unwritten ranges (VIBEFS.md §15's Extents row); `sync_file_range` with `SYNC_FILE_RANGE_WAIT_BEFORE` or `SYNC_FILE_RANGE_WAIT_AFTER` returns writeback errors per open file description, and `syncfs` its volume's, as §12.5's writeback-error box records them
- [ ] `FS_IOC_GETFLAGS`, `FS_IOC_SETFLAGS`, `FS_IOC_FSGETXATTR`, and `FS_IOC_FSSETXATTR` on vibefs v2, over VIBEFS.md §15's Inode flags row, as `lsattr`, `chattr`, and systemd call them: immutable and append-only enforced as Linux enforces them and changed only with `CAP_LINUX_IMMUTABLE` (§18.6), NOCOW set only on an empty file or inherited from its directory, and a flag vibefs does not store refused as Linux refuses it; in-guest, after `chattr +i` from the §14.9 mirror a file refuses writes, unlink, and rename until `chattr -i`
...
- [ ] block devices with `size`, `ro`, `removable`, `queue/logical_block_size`, `queue/io_timeout` (the §12.5 request deadline in milliseconds, writable, as Linux's), and, per partition, `partition` and `start`, which `lsblk` and udev read; `BLKRRPART` (built by whichever of §22.2 and §23.3 lands first, with §22.2's `EBUSY` rule) rescans a disk's partitions with their uevents; a virtio-blk capacity change (QEMU's `block_resize`) updates `size` and sends `change` with `RESIZE=1`
...
direct entry. Phase 27 needs only 18, 19, and 20, so it can start beside 21 and 22. Phase 28 needs 25
...
- [ ] ten panic-and-reset cycles with `panic=1`, each on the same variable store, under OVMF and under the aarch64 edk2 build, on the nightly job: each boot logs the record of the cycle before, efivarfs lists at most two panic-record variables after any cycle, and `QueryVariableInfo`'s remaining space, which each boot logs, never falls below 5 KiB; a `BootNext` write through §20.9's efivarfs then succeeds. The firmware may reclaim the space of deleted variables only at a later boot, so the line bounds the records present and the floor, not a return to the first cycle's free space. The same run on x86_64 `q35` with `acpi-erst` leaves at most two ERST records. A third record after any cycle shows the store still accumulates, and remaining space under 5 KiB shows the free-space rule failing; if a firmware refuses the deletes, the EFI backend falls back to one variable name rewritten in place. Phase 26's cloud firmware images, whose variable stores are small, exercise the 5 KiB rule
...
- [ ] the §25.7 SQLite test, in WAL mode with `synchronous=FULL` on §10.2's volatile-cache device, loses no acknowledged transaction across 1000 simulated power cuts, on both architectures, on the weekly job in shards of at most 5.5 hours
...
- [ ] the `#MC` handler, entered on its §2.1 IST stack and moved to the thread's kernel stack first when it interrupts ring 3 (DESIGN §5.10 rule 3), reads `MCi_STATUS`, `MCi_ADDR`, and `MCi_MISC`, grades severity (corrected, UCNA, SRAO, SRAR, fatal) by Intel's encoding, and by AMD's `Deferred` and `Poison` bits on AMD CPUs, and takes no lock that normal code holds
...
- [ ] the retired-frame list kept across reboots in one EFI variable (§20.9) of at most 64 entries, written under DESIGN §2.5's free-space rule and applied before any user page is allocated: it is keyed by the SMBIOS system UUID (§25.2) and a hash of the boot memory map, each entry carries its DIMM locator where SMBIOS type 17 names one, and a boot whose key differs discards the list with a log line naming the reason; when the list is full, a newly retired frame stays retired for this boot and the log says it was not kept. In-guest, a frame soft-offlined by repeated `mce` injections is retired again before `/sbin/init` after a restart on the same variable store, and a restart with a different `-m` discards the list
...
- [ ] one direct entry per architecture, taken by every boot without Limine, which does for its own image what Limine does for a firmware boot (DESIGN §4.1): it draws the slide, keeping the image in the top 2 GiB, and the HHDM offset, so that every RAM-typed range fits, inside its own DESIGN §4.1 slots; applies the image's §18.2 relocations; builds the tables the kernel starts on (the image at the slid base, the HHDM over every RAM-typed range, and an identity map of the entry); and records the base, the offset, and a seed for §18.2's region bases in `BootInfo`, which stays the only boot input the kernel reads, so a kernel started by kexec needs neither Limine nor firmware. The seed is the one the start record carries (a serialized `BootInfo`'s §14.7 seed, or a device tree's `/chosen/kaslr-seed`), or else one from `RDSEED` or `RDRAND` on x86_64 and `RNDR` on aarch64, and zero, logged, on a CPU with none. The entry's stack and fixed tables are sections of the image's load extent, which it zeroes before use, so any loader that places the image places them; the HHDM's tables come from RAM that the map it was handed types usable and that the identity map it runs on covers, and the entry types what it took as bootloader-reclaimable in `BootInfo`, as Limine types its own tables. An in-guest test kexecs, in both directions, between two builds of the current tree whose DESIGN §4.1 layouts differ, a `kernel_tests` build moving the physmap slot by 1 TiB, and both reach `shell ready`, on both architectures; a failure the entry-state section cannot describe, where the new kernel needs something only the loader knows, reopens DESIGN §4.1's rule that the loader draws no layout
- [ ] x86_64: the direct entry is one 64-bit body with two doors. §26.4's PVH door enters it from 32-bit protected mode. The 64-bit door, at an offset from the image's load base that a `vibeos` ELF note in the image records, is entered in long mode with interrupts off, on an identity map of every RAM-typed range, with the serialized `BootInfo`'s physical address in `rsi`, as Linux's kexec enters a bzImage at `startup_64`. The body builds its tables at the paging depth it needs and changes the depth itself when the door's differs (§27.6)
- [ ] aarch64: the direct entry sits behind a Linux arm64 `Image` header whose `image_size` covers its load extent. It is entered at a 2 MiB-aligned address with the MMU and D-cache off, at EL2 or EL1, with `x0` holding the physical address of a device tree from a VMM or of a serialized `BootInfo` from kexec, told apart by their magics, and `x1` to `x3` zero, as Linux's arm64 booting document states; from a device tree it builds `BootInfo` through §11.5's parser. Entered at EL2, it leaves the CPU as Limine does (VHE where the CPU has it, else EL1), so §11.1's rule that the kernel never changes exception level holds from `_start` on. Before it turns the MMU on, it invalidates the cache lines its MMU-off writes cover, as Linux's `head.S` does
- [ ] the serialized `BootInfo`, and §30.6's handover table beside it, are a versioned format documented in `docs/` before any code, because a kernel hands them to a different kernel version, older or newer (§30.4's update reboots, §30.6's live update): a magic, a format version, and length-prefixed tagged records, each tag marked required or optional; a kernel skips an optional tag it does not know and refuses a handover with an unknown required tag, with a named reason on serial, before it touches memory. On aarch64 with GICv3, a required tag, present from the format's first version, carries the ranges of the LPI property and pending tables (DESIGN §5.4): a kernel that finds `GICR_CTLR.EnableLPIs` set reuses those tables, never writes `GICR_PROPBASER` or `GICR_PENDBASER`, and keeps the ranges out of its buddy allocator, as Linux does on GICs that cannot clear the bit. Both join §22.1's list of kept formats from the release that first writes them, so within a major version a release adds only optional tags. Host tests parse the stream each earlier release keeps in §22.1's format corpus and a stream with an unknown optional and an unknown required tag, and an in-guest test kexecs from the release before to the current kernel and back; on aarch64 that test runs under `virt,gic-version=3`, and the second kernel's MSI-X interrupts arrive with EnableLPIs left set by the first
- [ ] the format document gains an entry-state section, written before any entry code, because the state a kernel is entered in crosses versions as the records do: for each door of the direct entry, the CPU mode and exception level, what the tables it is entered on map and with which attributes, which register holds the start record, where the image sits and how it is aligned, and the rule that every range the entry state uses (the image's load extent, the initrd and any other module, the command line, the start record or serialized `BootInfo`, purgatory and its identity map, and §30.6's handover ranges) is typed executable-and-modules or bootloader-reclaimable in the map handed over, never usable. Xen's PVH ABI, Linux's x86 64-bit boot protocol, and Linux's arm64 booting document are its behavioural references for the PVH, 64-bit, and aarch64 doors
- [ ] `kexec_file_load` places bytes and passes a record, and draws no layout: it places the image's load extent at a 2 MiB-aligned physical address, loads the initrd and the command line, and writes the serialized `BootInfo` with a fresh §14.7 seed, typing every range of the entry state as the entry-state section requires. It builds none of the new kernel's tables, because the image's own direct entry draws its layout (DESIGN §4.1) and the loader runs the release before, which cannot know the new release's DESIGN §4.1 table. On x86_64 it builds only the identity map that purgatory and the 64-bit door run on, at load time, and for a crash load inside the `crashkernel=` region, since the panic path maps nothing (§22.2)
- [ ] `shutdown` on the §6.1 `Driver` trait, the same quiesce function as the stop step of `remove` and run in DESIGN §12.2's order, children and consumers first, quiescing DMA on every device before the jump, a block driver through DESIGN §10.3's queue quiesce and its stop step, since a NIC still writing into the old kernel's buffers corrupts the new one
...
- [ ] x86_64 entry through a purgatory that checks the loaded image's digest and then, still in identity-mapped long mode, jumps to the direct entry's 64-bit door; aarch64 entry into the aarch64 direct entry with the MMU and caches off at the entry exception level, after cleaning to the point of coherency
- [ ] a capture region reserved at boot by a `crashkernel=` option on the §10.2 command line; with a capture kernel loaded, the panic path stops every other CPU through DESIGN §2.5 step 1's stop primitive, the one a plain panic uses, then jumps into the capture kernel; DESIGN §2.5 records the jump
...
- [ ] the request word §10.7 gave the NMI handler gains BACKTRACE and LOCKUP beside STOP, which the buddy check above and the all-CPU backtrace below set before they send the NMI IPI; on aarch64 the GICv3 pseudo-NMI above becomes the NMI step of DESIGN §2.5 step 1's stop, and §10.7's `panic_test` variant then requires `cpu 4 stopped (nmi)` under `virt,gic-version=3` and still `cpu 4 not stopped` under `virt,gic-version=2`; an NMI with no request pending, on a CPU that is neither stopped nor the dump owner, is external: on x86_64 the handler first reads port `0x61`'s SERR and IOCHK bits into the report, then the CPU that takes it queues an all-CPU backtrace (the box below) and returns, printing nothing in NMI context (DESIGN §2.2), where today every NMI halts the kernel (`arch::idt`'s `nmi` handler)
...
- [ ] the §3.4 blocked-thread sweep covers every blocked thread, with a deadline or without one, and reports a thread blocked longer than a threshold (at least the largest stall bound S among registered block devices, DESIGN §10.3, since a thread waiting for a lock may wait behind another thread's I/O) with its stack and the lock or wait queue it is on, not only its id; an in-guest test blocks a thread with no deadline on a wait queue nothing wakes and finds the report naming that queue; today the sweep, which §10.7 made fire, sees only waiters with a deadline (F111)
- [ ] `/dev/watchdog` with Linux's ioctls (`WDIOC_KEEPALIVE`, `WDIOC_SETTIMEOUT`, `WDIOC_GETTIMEOUT`, `WDIOC_GETBOOTSTATUS`) over §20.6's watchdog drivers (`i6300esb`, the ICH9 TCO timer on `q35`, and the SBSA generic watchdog on `sbsa-ref`); init (§14.3) keeps feeding it after §22.2's health check commits, and stops when a critical service misses its own heartbeat for longer than its allowance, the service's heartbeat interval plus at least half the watchdog timeout, so a storage stall within S (DESIGN §10.3) never counts as a missed heartbeat
...
- [ ] the reset reason (watchdog, panic, machine check, power) reported at boot from the persistent record or the watchdog's status; a panic record present at boot, which the boot before would have deleted once it logged it (DESIGN §2.5), wins over the watchdog's status, since a panic that halts is then reset by the watchdog; §22.2's trial accounting takes its cause from it, so a watchdog reset or a machine check during a trial records the update as failed at once, and a power reset is armed again as a reset with no recorded cause is, before or after the trial's root
...
- [ ] `vibeos.counter_wrap=1` on the §10.2 command line, which starts every registered counter one minute before its wrap, run as a scheduled test variant
- [ ] vibefs v2 `fsync` fast enough for Phase 30's database line: if §14.8's recorded verdict calls for it, because in some series of its record v2's p50 is above 1.3 times ext4's or v2's p99 is above 3 times ext4's (1.3 keeps a margin under the 1/0.7 that Phase 30's 70% gate leaves a commit-bound client), v2 gains VIBEFS.md §15's intent log, the compat feature `intent_log` with its incompat `log_replay` bit, so an `fsync` writes that file's changes as one log record and flushes once, and mount replays the log after the newest superblock. §12.5's crash-state enumerator covers the log; host tests find that a v2 mount which treats `intent_log` as unknown mounts a cleanly unmounted volume that has it, commits to it, and leaves the log's blocks allocated, and refuses a volume whose `log_replay` bit is set; and the SQLite line below runs with it. Otherwise this box closes on §14.8's record and verdict, with the numbers. If it closes without a log and Phase 30's pgbench gate then misses 70% of Linux while the PostgreSQL-pattern p50 was within 1.3 times ext4's, the threshold was too loose, and this box reopens
- [ ] a test program over SQLite from the §14.9 mirror runs WAL-mode transactions against §10.2's volatile-cache device, cuts power at a random point in each iteration, and checks that every acknowledged transaction survived
...
- [ ] on first boot under every VMM above, and under QEMU on both architectures, the image grows its root partition (the state partition in §22.2's layout) and vibefs v2 filesystem to a disk enlarged since the image was built, and generates per-instance SSH host keys whose fingerprints it prints on the console; afterwards the disk's GUID, its partitions' unique GUIDs, and each vibefs volume's `uuid` differ from the image's. Under QEMU on both architectures, with the guest's own disk first in the boot order: a guest whose disk has booted once boots its own root with a never-booted copy of the image attached as a second disk, and mounts that copy by device path; with a byte copy of its own booted disk attached instead, its boot is refused with a line naming both disks (DESIGN §10.5), and after the runner gives the copy new ids (`tune-vibefs --uuid` on each vibefs partition, and `sfdisk --disk-id` and `--part-uuid`), the guest boots its own root. A cloud's rescue flow in which DESIGN §10.5's rule refuses a collision that Linux handles safely reopens the rule
...
### 26.4 MicroVMs
- [ ] a PVH entry, the `XEN_ELFNOTE_PHYS32_ENTRY` note in the kernel ELF, that builds `BootInfo` (§10.3) from the PVH start info's memory map, command line, and modules, as §25.4's direct entry builds it from a serialized one, so Firecracker, cloud-hypervisor, and QEMU's `microvm` start the kernel directly with no firmware and no Limine; with no firmware, TPM, or Secure Boot, the §18.7 check prints `boot: unverified`, and the microVM relies on its VMM, which DESIGN §2.10 trusts for everything; before the body takes any frame, the image's load extent, the start info, the command line, and each module are typed as §25.4's entry-state section requires, whatever type the VMM's E820 map gives their ranges
- [ ] the PVH entry, which the VMM starts in 32-bit protected mode with paging off and `ebx` holding the PVH start info's physical address, is §25.4's x86_64 direct entry through its PVH door: 32-bit code in a section of its own that only enters long mode on a fixed identity map of the low 4 GiB, built in the image's load extent, and then joins the body that §25.4's 64-bit door runs, which draws the slide and the HHDM offset, applies the image's §18.2 relocations, builds the tables, and records both offsets and a seed for §18.2's region bases in `BootInfo`; the start info carries no seed, so the seed comes from `RDSEED` or `RDRAND`, zero, and logged, on a CPU with neither
...
### 26.6 Images and CI
- [ ] the §22.1 release emits a raw disk image, a Google `disk.raw` tarball, a fixed-size VHD aligned to 1 MiB, and the kernel ELF with its PVH note for direct boot, byte-reproducible like its other artifacts; its vibefs volumes are built with `mkfs-vibefs --template`, and their ids and the disk's GUIDs are derived from the image, as §10.2 derives the ISO's, so §26.2's first boot replaces them
...
- [ ] x86_64 vectors allocated per CPU rather than from one global pool, since one MSI-X vector per queue per CPU exhausts a single space of about 200; a change inside the x86 `IrqChip`, since drivers name an `IrqId` (DESIGN §5.4)
...
- [ ] compaction: movable pages migrated through §12.1's reverse map to rebuild free 2 MiB blocks, on demand and in the background, each old unit released through the gather once the invalidation of its PTEs completes (DESIGN §2.4); a page that §21.2 maps into a guest invalidates the VM's second-level entry before it moves (DESIGN §2.4)
...
- [ ] the SR-IOV capability: virtual functions enabled, their BARs sized, and bound like any PCI device; enabled through Linux's sysfs files in the physical function's §23.3 directory, as `Documentation/ABI/testing/sysfs-bus-pci` defines them: `sriov_totalvfs`, `sriov_numvfs` (writing N enables N functions, writing 0 disables them, and writing another nonzero N while functions are enabled fails with `EBUSY`), and `sriov_drivers_autoprobe`, with a `virtfn<N>` link to each virtual function and a `physfn` link back
- [ ] per-VF MAC and VLAN filters set from the physical function: through rtnetlink on the physical function, as `ip link set <pf> vf <n> mac <addr> vlan <id>` from the §14.9 mirror sends them (`RTM_SETLINK` with `IFLA_VFINFO_LIST` holding `IFLA_VF_MAC`, `IFLA_VF_VLAN`, and `IFLA_VF_VLAN_LIST`), and reported in `RTM_GETLINK` replies with `IFLA_NUM_VF` and, when the request's `IFLA_EXT_MASK` has `RTEXT_FILTER_VF`, `IFLA_VFINFO_LIST`, each attribute's layout pinned by a host test that cites `include/uapi/linux/if_link.h`; in-guest, `ip link` sets a VF's MAC and VLAN, and `ip link show` reads back the MAC and VLAN it reads under the §23.6 reference kernel booted with the same root and `igb` device
- [ ] device assignment to a §21.2 guest through Linux's VFIO interface, so QEMU's `vfio-pci` uses it with `host=<bdf>`: `/dev/vfio/vfio` and a `/dev/vfio/<n>` node per §18.1 isolation group; an `iommu_group` link in each assignable device's §23.3 `/sys/bus/pci/devices/<bdf>/` directory pointing to `/sys/kernel/iommu_groups/<n>/`, whose `devices/` lists the group; binding to VFIO through the device's `driver_override` and `/sys/bus/pci/drivers/vfio-pci/bind`, as on Linux; Linux's type-1 VFIO API as `include/uapi/linux/vfio.h` and `Documentation/driver-api/vfio.rst` define it: `VFIO_GET_API_VERSION`; `VFIO_CHECK_EXTENSION`, reporting `VFIO_TYPE1v2_IOMMU` and no extension vibeOS lacks; `VFIO_SET_IOMMU`, `VFIO_IOMMU_GET_INFO`, `VFIO_IOMMU_MAP_DMA`, and `VFIO_IOMMU_UNMAP_DMA` on the container; `VFIO_GROUP_GET_STATUS`, `VFIO_GROUP_SET_CONTAINER`, `VFIO_GROUP_UNSET_CONTAINER`, and `VFIO_GROUP_GET_DEVICE_FD` on the group; and `VFIO_DEVICE_GET_INFO`, `VFIO_DEVICE_GET_REGION_INFO`, `VFIO_DEVICE_GET_IRQ_INFO`, `VFIO_DEVICE_SET_IRQS`, and `VFIO_DEVICE_RESET` on the device, whose regions are read, written, and mapped at the offsets `VFIO_DEVICE_GET_REGION_INFO` reports; each ioctl's number and argument layout pinned by a host test that cites the header; iommufd (`/dev/iommu` and the VFIO device nodes under `/dev/vfio/devices/`) is absent and listed in `docs/LINUX.md`, so QEMU's `vfio-pci`, which uses the container unless given an `iommufd` object, takes that path; the group mapped to guest memory, and its MSI-X delivered to the guest through §21.2's `irqfd`; on aarch64, DESIGN §11.5's SError row gains, in the same commit, how an SError that an access to an assigned device's memory from EL0 or from a guest raises is charged, since the row halts on every SError today
...
- [ ] a wedged member under the root: with §22.2's watchdog armed at a configured 10 s and §25.5's blocked-thread threshold at 5 s, a `kernel_tests` hook stops one member of the root's RAID 1 array of two virtio-blk disks from completing mid-workload and makes its reset fail; md fails that member within the member's stall bound S (DESIGN §10.3), the workload sees no error, and no watchdog reset and no blocked-thread report follow, since either would mean a detector sits below S. Each member's `queue/io_timeout` is 2000 ms, and the test declares the member's timeout and reset lines; under TCG with 2 vCPUs and 1 GiB, on both architectures
- [ ] 1000 simulated power cuts during RAID 5 and RAID 6 writes on §10.2's volatile-cache device, each followed by reassembly and the bitmap-bounded resync, leave no stripe whose parity disagrees with its data; 1000 more during degraded writes with the §29.2 journal leave every block the interrupted writes did not touch intact; on the weekly job in shards of at most 5.5 hours
...
- [ ] the §29.4 vibefs v2 scrub finds a block the host corrupted underneath either member of a RAID 1 array, rewrites it from the copy whose checksum verifies, and counts and logs the repair; with the block corrupted underneath the first member, the §29.4 scheduled scrub (md's `check`, then the vibefs scrub) leaves both members holding the good data, under TCG with 2 vCPUs and 1 GiB, on both architectures
- [ ] on a RAID 5 array of four virtio-blk disks, a data block the host corrupted underneath one member is found by the §29.4 vibefs v2 scrub, rebuilt from the other members, and rewritten with its stripe's parity unchanged, and parity the host corrupted in another stripe, under data the scrub verifies, is recomputed by the scheduled scrub's `repair`, so a second `check` counts no mismatch; under TCG with 2 vCPUs and 1 GiB, on both architectures
...
- [ ] vibefs v2 on §18.7's encryption on a logical volume on RAID 1, each member its own §10.2 volatile-cache export, passes §12.5's crash-state enumeration and content oracle, each state assembled by §29.3's host stack reader: every state mounts a committed generation at or after the last acknowledged `fsync`, under TCG with 2 vCPUs and 1 GiB, on both architectures; a QEMU kill alone cannot show a missing flush (F080)
...
- [ ] a mapping-target interface for stacked block devices (linear, striped, mirror, parity, thin, and §18.7's `crypt` target), so RAID, volumes, and encryption stack through one mechanism; userspace drives it through Linux's device-mapper ioctls on `/dev/mapper/control` (`DM_VERSION`, `DM_REMOVE_ALL`, `DM_LIST_DEVICES`, `DM_DEV_CREATE`, `DM_DEV_REMOVE`, `DM_DEV_RENAME`, `DM_DEV_SUSPEND`, `DM_DEV_STATUS`, `DM_DEV_WAIT`, `DM_TABLE_LOAD`, `DM_TABLE_CLEAR`, `DM_TABLE_DEPS`, `DM_TABLE_STATUS`, `DM_LIST_VERSIONS`, and `DM_TARGET_MSG`, with `DM_VERSION` reporting no newer an interface version than is implemented, since libdevmapper picks its calls by it) with the `linear`, `striped`, `raid`, `thin-pool`, `thin`, and `crypt` target names, so unmodified `dmsetup`, LVM2, and `cryptsetup` from the §14.9 mirror run; a stacked device completes its parent requests and runs failover and mirror retries as stage-2 items (DESIGN §10.1), never in a member's bottom half; this interface extends §18.7's calls and `crypt` target rather than adding a second key path; `DM_LIST_VERSIONS` reports for `crypt` the target version whose table options are all implemented, which `docs/LINUX.md` records, and cryptsetup, finding no kernel keyring (`add_key`, `request_key`, and `keyctl` return `ENOSYS`), passes the `crypt` key in the table; in-guest, unmodified cryptsetup opens a volume the §18.7 tool formatted, and the tool opens one cryptsetup formatted
- [ ] copies through the mapping-target interface, which is vibeOS's own and which device-mapper's ioctls do not carry: each target reports how many distinct copies of a range it can return, and serves a read that names a copy and a rewrite that names the stored copy to replace. A mirror's copies are its members' copies, and RAID 10's are those of the mirror set that holds the range; a RAID 5 range has two, the data as stored and the data rebuilt from the other members, and a RAID 6 range three, the data as stored and the data rebuilt through P and through Q; a rewrite on parity RAID writes the stored data alone and leaves parity as it is, since the parity was computed from the good data; a degraded array counts only the copies its present members can produce. A linear, striped, thin, or crypt target passes on the count and the index of the child that holds the range, since requests split at chunk boundaries (below), and crypt decrypts the copy a read names and encrypts the data a rewrite writes, so vibefs compares plaintext checksums. Host tests read and rewrite each copy of a range on RAID 1, 5, 6, and 10, directly and through a crypt target, and find parity unchanged after a rewrite on RAID 5 and 6
...
- [ ] arrays and logical volumes listed in §23.4's `/proc/diskstats` and under `/sys/block` with `slaves/` and `holders/` links rendered from the DESIGN §12.1 supplier links each member records, as Linux lists `md` and `dm` devices, so `iostat` and `lsblk` see the stack; per-device latency histograms, which Linux exposes only through eBPF, in debugfs under `block/<dev>/vibeos/`, beside Linux's own `block/<dev>/` files
- [ ] native NVMe multipath: a namespace that a subsystem shares between controllers is one block device named as Linux names it, with a path per controller, I/O retried on another path when a request on one completes with an error, its controller's timeout and recovery included (DESIGN §10.3), and the subsystem and its controllers under `/sys/class/nvme-subsystem`, as Linux lists them
- [ ] zoned namespaces through Linux's zoned block interface: `BLKREPORTZONE`, `BLKRESETZONE`, `BLKOPENZONE`, `BLKCLOSEZONE`, `BLKFINISHZONE`, `BLKGETZONESZ`, and `BLKGETNRZONES`, and `queue/zoned`, `queue/chunk_sectors`, and `queue/nr_zones` under `/sys/block`; at most one write in flight per sequential zone, the next held back until the one before it completes (DESIGN §10.2), and zone append where the device offers it
- [ ] §12.5's error handling through stacked devices: a stacked device has no deadline of its own (DESIGN §10.3); when a member request completes with an error, its member's timeout and recovery included, the stacked device, in a stage-2 item (DESIGN §10.1), fails over to another path (NVMe multipath below) or fails that member of the array (§29.2), never the array, and a stacked request completes within the stacked device's stall bound, the sum over the members it can try in turn
- [ ] arrays and volumes found by §20.9's persistent names and by filesystem UUID, never by probe order; a name or UUID that more than one device carries is refused with a line naming each (DESIGN §10.5), and the device path still finds the volume
...
- [ ] the §10.2 volatile-cache device serving each member as its own export, with one power cut applied to all of them, which the RAID power-cut gate runs on
...
- [ ] a host-side stack reader in hostlib, beside `fsck-vibefs`, that assembles a vibefs volume from member images without root, on Linux and on macOS (the hosted runners and §10.2's scheduled macOS job): RAID 1 through §29.2's portable md 1.2 code, reading the member md's resync would copy from; LVM2 linear and striped segments mapped from the physical volumes' text metadata, host-tested against volume groups LVM2 created under Linux; §18.7's LUKS2 volumes opened through the unlock tool's portable crate, host-tested against volumes that cryptsetup created under Linux and volumes the tool formatted; `fsck-vibefs` and the §12.5 enumerator take the assembled volume, which the Phase 29 host checks run on

### 29.4 Integrity and health
- [ ] scrub on a schedule and on demand: md's `check` and `repair` through `sync_action`, with mismatches counted in `mismatch_cnt` and logged; `repair` rewrites parity from data and mirrors from the first member, as Linux's does, since a mismatch alone does not say which copy is right. The scheduled scrub runs `check`, then the vibefs v2 scrub below over each filesystem on the array; on a RAID 5 or 6 array it then runs `repair`, but only when `check` counted mismatches and every filesystem on the array checksums its data and finished its scrub with each block verified or rewritten, so `repair` recomputes only parity that disagrees with verified data; on a mirror, a mismatch left after such a scrub lies in blocks no file uses and is logged. `repair` on demand stays an administrator's action: the kernel logs a warning when `repair` is written to the `sync_action` of an array under a filesystem that checksums its data, since there it can copy a corrupt block over the good one, and `docs/` says why
- [ ] a vibefs v2 checksum failure, or a metadata block that fails its pointer's check (VIBEFS.md §15), retries the read from another copy through §29.1's copy index and rewrites the bad copy; a vibefs v2 scrub reads every copy of a mirrored block that way, and the stored copy of a block on parity RAID, trying its rebuilt copies only when that fails, since md's `check` already counts parity that disagrees with its data, so a corrupted copy is found and rewritten from one that passes those checks, whichever member holds it; the retry and the rewrite run as stage-2 items (DESIGN §10.1), and a host test fails the retry's request allocation and finds the original read completing with its checksum error
...
- [ ] vibefs v2 grown online into new space at the end of its device, without unmounting, by a commit that raises `nblocks` (VIBEFS.md §15's Superblocks and geometry row)
...
- [ ] per-user, per-group, and per-project quotas enforced and managed through Linux's `quotactl`, with project ids set through §23.1's `FS_IOC_FSSETXATTR` and stored as VIBEFS.md §15's Inode flags row says
...
- [ ] series names documented and stable across releases, with any change listed in the changelog until `v1.0.0`, when §39.1 freezes them
...
- [ ] a render device shaped like Linux's `vgem`, enabled by `vibeos.vgem=1` on the §10.2 command line, as §32.2's display is: GEM buffers in system memory, mapped and exported as dma-bufs, with its fence-attach and fence-signal ioctls, which IGT's `vgem_*` and `prime_vgem` tests drive
...
- [ ] the §18.7 unlock tool's passphrase prompt, before root is mounted, in the configured keyboard layout
...
### 38.3 Protocols
- [ ] the vibefs v2 commit (§14.8), and §25.7's intent log when §14.8's recorded verdict calls for one, against a disk that completes writes in any order and, at a power loss, keeps any subset of the writes that no completed `Flush` covers, a `Flush` covering only writes that completed before it was submitted, tears a superblock write, completes a write or flush with an error, acknowledges a write it never performed, and loses power at any step: the mounted tree is a committed generation at or after the last acknowledged `fsync`, except that a block a never-performed write left stale is detected, not recovered: a mount whose newest superblock names a stale root takes the older slot's generation, logs the loss, and commits next at a generation above the rejected one, and a read that reaches a deeper stale block returns `Corrupt` (VIBEFS.md §15's Verified pointers and Superblocks and geometry rows); at every instant, every block the superblock a mount would pick reaches is intact, including a superblock whose write reported an error but reached the media, and so is every block the older slot's superblock reaches until a newer commit's superblock replaces it, a block a never-performed write left stale excepted; a failed commit leaves the in-memory generation where it was, and its retry writes the slot the failed commit wrote; and after any sequence of commits, failures, and remounts, every block the allocation map marks used is reachable from the newest committed generation or a snapshot (F049, F050)
...
- [ ] litmus tests for the barrier sequences the aarch64 `arch` code writes by hand: the §11.2 PTE update with TLB maintenance, break-before-make, and the §11.2 ASID switch, whose local `tlbi vmalle1` completes before the CPU loads an ASID of a new generation
...
- [ ] from the `v1.0.0` release candidate on, each release candidate and each release checks in a copy of the generated syscall table (§10.5), and the release workflow refuses one without it; `make check` compares the table with the last release's copy, or, until `v1.0.0` is tagged, with the newest candidate's, and fails on a removed entry, a changed number, or a changed argument layout, unless a row added to `docs/LINUX.md`'s baseline log since that copy names the change: a baseline move (§39.1), a layout corrected to the baseline's uapi definition, with that definition and the conformance case that showed the old layout wrong, or, in a major version, the removal of an interface a move left behind; a host test fails when a number the table marks implemented reaches the dispatch's `ENOSYS` arm, or a number it does not mark implemented reaches a handler, so the frozen copy is the ABI the kernel serves (F150)
- [ ] the §39.2 ABI corpus runs unchanged on every push to `main`, on both architectures
- [ ] the §22.1 format corpus written by the release candidate passes the §39.2 checks on `main` in CI
- [ ] an unattended A/B update (§22.2) from each supported release, and from Phase 22's release, to `main` boots and passes `make test-e2e`; afterwards the previous release, booted from its slot through the firmware menu, mounts the state partition, keeps the user and files the update test made, and passes `make test-e2e`, nightly, on both architectures under QEMU with OVMF and the aarch64 edk2 build (TCG, 2 vCPUs, 2 GiB)
...
- [ ] the Linux surface in the stable set, per architecture: the calls, flags, and `ioctl`s vibeOS implements, as the §10.5 table and the §13.9 `ioctl` registry list them, their errno values, the initial stack and auxv (§13.10), signal frames (§13.8), the process core files the §13.8 `core` action writes, in Linux's note layouts (not the §10.7 kernel dumps), the vDSO's symbol names and versions (§13.10), the `/proc` and `/sys` files of §13.10 and Phase 23, and the §16.1 DRM/KMS and §16.4 evdev nodes; for these, stable means they behave as the baseline `docs/LINUX.md` names, and the baseline is their version. A change that brings vibeOS closer to the baseline is a fix, not an interface change. The baseline moves only forward, to a later Linux longterm series, in a minor release whose notes and `docs/LINUX.md` baseline log list every interface the move changes. A move adds calls, flags, fields, and files, and changes behaviour only where Linux changed it between the two series; an interface Linux removed in the new series stays served, as a `docs/LINUX.md` deliberate difference, until vibeOS's next major version, and the release before that logs its use once per process
- [ ] vibeOS's own formats in the stable set: the formats §22.1 keeps from Phase 22's release (vibefs v2, the §14.6 formats with their version, expiry, and root-set fields, the disk layout and install configuration, the LUKS2 header, service definitions, the password-hash string, the panic record's container, and those later phases added, such as §25.4's serialized `BootInfo` and §30.6's handover table), §30.2's documented series names, and the documented §10.2 command-line options
...
- [ ] a stable interface of vibeOS's own (the formats and Wayland extensions above, and each native interface `docs/LINUX.md` lists) changes only by an addition an older reader ignores, by a new version served beside the old one, or by a major version under the §22.1 policy; the Linux surface changes only as the Linux-surface box above says
- [ ] a stable interface is removed only in a major version, and only after one release that logs its use once per process: one of vibeOS's own, or a Linux interface that a baseline move left behind
- [ ] each release checks in the list of conformance cases it passed (§13.11, Phase 23); `make check` fails when an expected-failure list names a case on the last release's list; a commit that moves the baseline retakes Linux's results and passing sets (§13.11, §23.6) and adds no expected-failure entry for a case on the last release's list, so it carries the implementation of every changed behaviour such a case checks, and a case that fails on the new baseline's oracle leaves the passing sets instead
...
- [ ] the §22.1 format corpus of every release from Phase 22's on, read on `main` as §22.1's nightly job reads it: vibefs and FAT images mounted, read, written, and checked with `fsck`, packages and a signed repository installed, upgraded, and removed, and every other sample read by the code that reads it; volumes that `mkfs` on `main` writes with its default features mount, read, write, and pass `fsck` on each supported release; and `main`'s updater, starting from each release's key-set record, follows the chain of key-set records to the current one
- [ ] a corpus failure fails the push like any other test; a deliberate break carries a §22.1 major version bump in the same commit; in the commit that moves the baseline, a corpus case whose result changes the same way under the new baseline's oracle kernel is Linux's change, not a vibeOS break: that commit runs the corpus under the new oracle and records each such case and its new result in `docs/LINUX.md`'s baseline log, the corpus tier expects the new result from then on, and no major version follows
...
- **vibefs clones** (after 14): VIBEFS.md §15's ro_compat `clone` feature, a reference-count tree keyed by physical block with an entry only for a shared block, so `FICLONE`, `FICLONERANGE`, and `copy_file_range` share extents as they do on btrfs and XFS; host tests clone a file, overwrite half of it, and find the other half's blocks counted in the tree and freed only when the last reference goes, beside the §14.8 snapshot tests.
