...
### 11.5 Devices
- [ ] the device tree from Limine's DTB response, parsed in the portable half and host-tested like the ACPI parser: CPUs, GIC and ITS (with each PCIe host's `msi-map` and each device's `msi-parent`, which give the ITS its DeviceIDs, DESIGN §5.4), timer, PL011, PL031, PCIe ECAM with its `interrupt-map`, virtio-mmio, `/psci`, and QEMU's `fw-cfg` node, so the §10.2 command line and the `vmcoreinfo` note use the same fw_cfg interface as on x86_64. RAM comes from the Limine memory map, never from `memory@` nodes, which Limine removes. `/chosen` is ignored as the protocol requires, so the console is the PL011 that `/aliases` names `serial0`, or, where the tree has no `/aliases` (QEMU before 9.1), the first node compatible with `arm,pl011` whose status is okay or absent, in tree order; host tests parse checked-in trees dumped with `-machine dumpdtb=` from QEMU 8.2, with and without `secure=on` (whose disabled secure UART comes first), and from a current QEMU, and each picks the UART at `0x9000000`; the parser fills the `MachineDesc` of the box below, and reads `/reserved-memory` and the FDT header's memory reservation block, whose ranges never enter the buddy, since Limine's memory map need not mark them; host tests cover a tree with both; DESIGN §7.1 gains the device tree's counterpart of its table list in the same commit
