...
- [ ] no fault or trap that ring-3 code raises reaches `exception_halt`: one table in the portable half, which `sig_for_vec` reads, gives each of vectors 0 to 31 its CPL-3 action, and a host test fails on a vector without one. `debug_ex` calls `try_user_fault` for a CPL-3 frame after the stub has swapped GS by CS.RPL and moved the frame from the IST stack into the thread's user frame, and the frame returns through the common return to ring 3, exit work included (DESIGN §5.10 rule 3), as Linux's `idtentry_mce_db` does, since the kill path can block; the stub saves DR6 into the frame and clears it before the move, and the body reads DR6 only from the frame (DESIGN §5.10 rule 9). `sig_for_vec` maps `#DB` to `SIGTRAP` and `#AC` to `SIGBUS`, and CPU init clears `CR0.AM` on every CPU. Today a user `popf` that sets TF, or an `int1` (`0xF1`), halts every CPU. In-guest, a child that sets TF with `popf` and a child that executes `int1` each end with `SIGTRAP` in their `wait4` status; a third child sets TF while a `kernel_tests` hook at the top of the CPL-3 `#DB` body yields, re-pins the thread to the other CPU, and clears TF in the saved frame, and the child then makes 100 `getpid` calls, at each of whose entries the hook finds `PerCpu.apic_id` equal to the running CPU's LAPIC ID, and exits 0; DESIGN §2.5 and §5.2 drop their F005 not-yet-enforced notes in the same commit (F005)
